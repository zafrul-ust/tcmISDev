...
function showWorkAreaGroupOptions(selectedFacility) {
	...
	if (workAreaGroupArray != null) {
		if (workAreaGroupArray.length == 0 || workAreaGroupArray.length > 1) {
			setOption(0, messagesData.all, "", "workAreaGroup");
		}
		...
		if (workAreaGroupArray.length == 1) {
			...
			for (var i = 0; i < workAreaGroupArray.length; i++) {
				...
				setOption(i+1, workAreaGroupDescArray[i], workAreaGroupArray[i], "workAreaGroup");
			}
		}
	}
}
...
//		setOption(0, messagesData.all, -1, "workArea");
...
function search() {
	if(!isInteger($v("searchArgument"),false)) {
		alert(formatMessage(messagesData.integer, $("searchField").options[$("searchField").selectedIndex].text));
