...
function showWorkAreaGroupOptions(selectedFacility) {
	...
	if (workAreaGroupArray != null) {
		if (workAreaGroupArray.length == 1) {
			setOption(0, workAreaGroupDescArray[0], workAreaGroupArray[0],
					"workAreaGroup")
		} else {
			for ( var i = 0; i < workAreaGroupArray.length; i++) {
				setOption(i, workAreaGroupDescArray[i], workAreaGroupArray[i],
						"workAreaGroup");
			}
		}
	}
...
/*
 * function generateExcel() { $("uAction").value = 'createExcel';
...
function showTransitWin(messageText) {
	var waitingMsg = messagesData.waitingforinputfrom + "...";
	$("transitLabel").innerHTML = waitingMsg.replace(/[{]0[}]/g, messageText);
	...
	if (!dhxWins.window("transitDailogWin")) {
		...
		transitWin.center();
		// setting window position as default x,y position doesn't seem to work,
		// also hidding buttons.
...
function nonHaasPurchasedChange(box) {
	if (box.checked) {
		box.value = 'Y';
		var searchField = document.getElementById("searchField");
		for ( var i = searchField.length; i > 0; i--) {
			searchField[i] = null;
		}
		setOption(0, messagesData.materialdesc, "MATERIAL_DESC", "searchField");
		setOption(1, messagesData.msds, "CUST_MSDS_NO", "searchField");
	} else {
		box.value = 'N';
		setOption(0, messagesData.containerprefix, "RECEIPT_ID", "searchField");
		setOption(1, messagesData.partno, "CAT_PART_NO", "searchField");
		setOption(2, messagesData.materialdesc, "MATERIAL_DESC", "searchField");
		setOption(3, messagesData.itemid, "ITEM_ID", "searchField");
		setOption(4, messagesData.msds, "CUST_MSDS_NO", "searchField");
	}
}
