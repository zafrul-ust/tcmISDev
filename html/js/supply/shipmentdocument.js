...
function resultOnLoad()
{
	...
	if (totalLines > 0)
	{
		document.getElementById("shipmentDocumentViewBean").style["display"] = "";

		initializeGrid();  
	}  
	else
	{
		document.getElementById("shipmentDocumentViewBean").style["display"] = "none";   
	}
...
function initializeGrid(){
	beangrid = new dhtmlXGridObject('shipmentDocumentViewBean');
...
function addNewShipmentDocument()
{
	
  var shipmentId = document.getElementById("shipmentId").value;
  var companyId = document.getElementById("companyId").value;
  var loc = "/tcmIS/supply/addshipmentdocument.do?shipmentId="+shipmentId+"";
  try {
 	children[children.length] = openWinGeneric(loc,"showaddNewMrDocument","600","300","no","100","100");
  } catch (ex){
 	openWinGeneric(loc,"showaddNewMrDocument","600","300","no","100","100");
  }
}
