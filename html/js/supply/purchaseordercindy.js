...
function populateRadianPo(selectedPo,selInvGrp) {
	...
	$("po").value = selectedPo;
	$("invenGrp").value = selInvGrp;
	$("invenGrpSpan").innerHTML = selInvGrp;
...
*/
	...
	var selectFob  =  document.getElementById("selectFob");
	...
	var selectPaymentTerms =  document.getElementById("selectPaymentTerms");
	...
	if (validSupplier.value == "No" || $v("supplierId") == "")
	{
	...
	if (!($v("po").length > 0)) //&& !(bpovalue.value.length > 0) )
	{
		finalMsgt = finalMsgt + " PO.\n" ;
	...
	if ($v("po").length > 0)   
	{
		...
		if (validShipTo.value == "No" || shiptoid1.value == "")
		{
	...
	for(j = 1; j <= noofRows; j++)
	{
		...
		{
			if ( (linechange == "Y") || (originallinestatus =="Unconfirmed") || (canassignaddcharge != "Y") )
			{            
...
function submitPurchaseOrder(actionName) {	
    ...
    {
    	var seqNumber = gridCellValue(lineItemGrid,j,'seqNumber');
    	var type  =  gridCellValue(lineItemGrid, j,'isMaterialTypeItem'); 
    	if (type == "N") {
    		if (window['chargesListGrid'])
    			window['chargesListGrid'].parentFormOnSubmit();
    	} else {    		
    		if (window['flowdownsGrid'])
    			window['flowdownsGrid'].parentFormOnSubmit();
    		if (window['lookAheadGrid'])
    			window['lookAheadGrid'].parentFormOnSubmit();		
    		if (window['specsGrid'])
    			window['specsGrid'].parentFormOnSubmit();
    	}
    }
...
function resendWBuyPO() {
	var action = document.getElementById("action");
	action.value = "resendWBuyPo";
	$("radianPo").value = $v("po");
	var nowofRows = (lineItemGrid.getRowsNum())*1;	
    for(j=1;j<=nowofRows;j++)
    {
...
function getShipTo() {	
    ...
    if ( validpo.value == "No" || (po.value == null || po.value == ""))
	{
...
function inventoryGroupSelected(invnGrpInfo) {
	//$("currency").value = invnGrpInfo.currencyId; //TODO: Check this? Invenotry group selection returns a default currency. What to do with it?
	$("opsEntityId").value = invnGrpInfo.opsEntityId;
	$("invenGrp").value = invnGrpInfo.inventoryGroup;
	$("invenGrpSpan").innerHTML = invnGrpInfo.inventoryGroup;
	headerChanged();
...
function getSupplier() {
	...
	if ( shipToId.value == "" || shipToId.value == null)
	{
		...
		loc = "/tcmIS/distribution/entitysuppliersearchmain.do?valueElementId=supplierId&statusFlag=true&displayElementId=supplierName&fromPurchaseOrderPage=Y&inventoryGroup="+inventoryGroup+"&opsEntityId="+opsEntityId+"&rowNumber="+rowNumber;
		openWinGeneric(loc,"Supplier","600","500","yes","50","50");	
	}
}

function supplierChanged(s) {
  	$("supplierLine1").innerHTML = s.returnSelectedValue;
  	$("supplierLine2").innerHTML = s.returnAddressLine1;
  	if (s.returnAddressLine2.length > 0) {
  		$("supplierLine3").innerHTML = s.returnAddressLine2;
  		$("supplierLine4").innerHTML = s.returnAddressLine3;
  	} else {
  		$("supplierLine3").innerHTML = s.returnAddressLine3;
  		$("supplierLine4").innerHTML = '';
  	}
  	$("mfgPhoneNo").innerHTML = s.returnMfgPhone;  	
  	$("supplierId").value = s.returnSelectedId;
  	$("suppRating").value = s.returnQualificationLevel;
  	$("suppRatingSpan").innerHTML = s.returnQualificationLevel;
  	//$("supplieremail").innerHTML = s.returnEmail;
  	//$("supplieremail").innerHTML = "<a href=mailto:"+s.returnEmail+">"+s.returnEmail+"</a>";
	$("paymentTerms").value = s.returnPaymentTerms;
	$("selectPaymentTerms").value = s.returnPaymentTerms;
	if (poLineSelectedRowId == null || poLineSelectedRowId == "") {
		poLineSelectedRowId = 1;
	}
	var inventoryGroup = $v("invenGrp");
...
function getSecondarySupplier()
{
    ...
    loc = "/tcmIS/distribution/entitysuppliersearchmain.do?secondarySupplier=Y&valueElementId=supplierid&statusFlag=true&fromPurchaseOrderPage=Y&displayElementId=supplierName&autoSearch=y";
	var secSuppId  =  gridCellValue(lineItemGrid, poLineSelectedRowId,'supplier'); 
    var invenGrp  =  $v("invenGrp");
    var opsEntityId  =  $v("opsEntityId");

    loc = loc + "&supplierName="+secSuppId;
...
function secondarySupplierChanged (s) {
	...
	$('secondarySupplierAddress').innerHTML = s.returnSelectedValue +" "+ s.returnAddressLine1 +" "+ s.returnAddressLine2 +" "+ s.returnAddressLine3;	
  	lineItemGrid.cellById(poLineSelectedRowId, lineItemGrid.getColIndexById("supplier")).setValue(s.returnSelectedId);
  	$("secondarySupplier").value = s.returnSelectedId;
}
...
function getOrderTaker() {
	if($v("supplierId").length == 0) {
		alert("Choose a supplier before choosing the contact.");
		return false;
	}
	invalidateOrderTaker();
    var loc = "/tcmIS/supply/posuppliercontact.do?action=search&supplier="+$v("supplierId")+"&contactId="+$v("orderTakerID")+"&displayElementId=supplierContactName&valueElementId=supplierContactId&fromSupplierPriceList=N";
    openWinGeneric(loc,"shoSupplierContacts","800","450","yes","50","50","no");
}

function setOrderTakerChanged(id, name, phone, fax, email ) {	
  	$("orderTakerID").value = id;
  	$("orderTaker").value = name;
  	$("contactPhoneNo").innerHTML = phone;
  	$("contactFaxNo").innerHTML = fax;
  	$("supplierContactEmail").innerHTML = email;
}
...
function carrierChanged(returnSelectedCarrierCode,returnSelectedCarrierName,returnSelectedAccount,returnSelectedCompanyHub, callbackparam) {
	$("carrierID").value = returnSelectedCarrierCode;
	$("carrieraccount").value = returnSelectedAccount;
	if (returnSelectedAccount != null && returnSelectedAccount != "" ){
		returnSelectedCarrierName = returnSelectedCarrierName + "/"+returnSelectedAccount;
	}
	$("carrierLine2").innerHTML = returnSelectedCarrierName;
	if(returnSelectedCompanyHub != null && returnSelectedCompanyHub != "")
		$("carrierLine1").innerHTML = returnSelectedCompanyHub;
...
function checkForChanges(rowId,oldRowId){
	var changes = 'N';
	if (oldRowId != 0 ) {
		changes = gridCellValue(lineItemGrid, oldRowId,'lineChange');
		lookAheadChanged = gridCellValue(lineItemGrid, oldRowId,'lookAheadChanged');
		
		if (changes == 'Y' || lookAheadChanged == 'Y') {
...
{	
	if (rowId != previousRowId) {	
		var changed = checkForChanges(rowId, previousRowId);
		if (changed) {
	...
	if (type == "N") {
		...
		$('lastconfirmedalt').innerHTML = gridCellValue(lineItemGrid, poLineSelectedRowId,'lastConfirmed');
		$('secondarySupplierAddress').innerHTML =gridCellValue(lineItemGrid, rowId,'secondarySupplierAddress');
		$('itemdescalt').innerHTML=gridCellValue(lineItemGrid, poLineSelectedRowId,'itemDescription');
		$('detailLineAlt').innerHTML = gridCellValue(lineItemGrid, poLineSelectedRowId,'poLineNumber');
	...
	//console.log("differentSupplierOnLine=" +differentSupplierOnLine +", secondarySupplierAllowed=" +secondarySupplierAllowed +", everConfirmed=" +everConfirmed +", changeSupplier=" +changeSupplier +", secondarySupplierAllowed=" +secondarySupplierAllowed);
	
	if ( differentSupplierOnLine == "Y" || secondarySupplierAllowed == "Y" && everConfirmed != "Y" ) {
		j$('#secondarySupplierAddress').show();		
		$("secondarySupplierEdit").style.display = "";
		j$("#secondarySupplier").attr('readonly', 'false');
		j$("#secondarySupplier").val(gridCellValue(lineItemGrid, poLineSelectedRowId,'supplier'));
		if ( changeSupplier == "N" || secondarySupplierAllowed != "Y") {
			j$("#secondarySupplier").attr('readonly', 'true');
			j$("#secondarySupplier").val("");			
			$("secondarySupplierEdit").style.display = "none";
		}
	} else {
		j$("#secondarySupplier").val("");
		j$("#secondarySupplier").attr('readonly', 'true');		
		$("secondarySupplierEdit").style.display = "none";
		j$('#secondarySupplierAddress').hide();
	}
...
function populatePoLineDetail(rowId){
	...
	j$('#secondarySupplierAddress').html(gridCellValue(lineItemGrid, rowId,'secondarySupplierAddress'));	
	...
	j$('#secondarySupplier').html(gridCellValue(lineItemGrid, rowId,'secondarySupplier'));
	//textarea
	...
	if (gridCellValue(lineItemGrid, rowId,'msdsRequestedDate') != null && gridCellValue(lineItemGrid, rowId,'msdsRequestedDate') == 'Y')
		j$('#sendMsdsCheckBox').prop( "checked", true );
	else 
		j$('#sendMsdsCheckBox').prop( "checked", false );
}
...
function setLineUnconfirmed(rowId)
{
	...
	if (document.getElementById("addbuyorderslink"))
		document.getElementById("addbuyorderslink").style["display"] = "none";
    lineqty = gridCellValue(lineItemGrid, rowId,'quantity');	
    qtyreceived = gridCellValue(lineItemGrid, rowId,'quantityReceived');	
    selectedRowStatus = gridCellValue(lineItemGrid, rowId,'poLineStatus');	
    selectedlRowStatus = gridCellValue(lineItemGrid, rowId,'status');	
...
{
	var seqNumber = gridCellValue(lineItemGrid,poLineSelectedRowId,'seqNumber');
...
{  
	document.getElementById("addbuyorderslink").style["display"] = "none";
    var validshipto  =  $v("validShipTo");
	var HubName  =  $v("hubName"); 
    ...
    if (validpo == "No") {
		 alert("Please Choose a Valid PO.");
    } else if ( (shiptoid.length > 0) && (validshipto == "No") ) {
	     alert("Please Choose a Valid Ship To.\nOr\nLeave the Input Field Blank.");
	} else if (invengrp.trim().length == 0) {
        ...
        if (radianPO.length > 0) {
			...
			} else if ( (shiptoid.length > 0) && (validshipto == "Yes")) {
...
function refereshEditAssociatedPrs() {
...
function addNewItem(lineId, itemType, isMaterialTypeItem) {
	var j = 0;
	var newId = lineItemGrid.getRowsNum();
	var newData = new Array();
	var dateEditablePerm = isMaterialTypeItem;
    lineItemGrid.selectRowById(newId, null, true, true);
	...
	newData[j++] = lineId + "/0";  //line - line/amendment
	newData[j++] = '';		//ghsCompliant
	newData[j++] = '';		//itemId
	newData[j++] = '<input name="lookupItemId" type="button" value="..." class="lookupBtn" onmouseover="this.className=\'lookupBtn lookupBtnOver\'" onmouseout="this.className=\'lookupBtn\'"  onclick="getItemDetail();"></c:if>';		//itemIdLookup
	newData[j++] = '';		//type	
	newData[j++] = dateEditablePerm;			//needDatePermission
	newData[j++] = '';		//needDate
	newData[j++] = dateEditablePerm;			//promisedDatePermission
	newData[j++] = '';		//promisedDate
	newData[j++] = dateEditablePerm;			//projectedDatePermission
	newData[j++] = '';		//projectedDate	
	newData[j++] = 'Y';		//quantityPermission
	newData[j++] = '';		//quantity
	newData[j++] = '';		//uom
	newData[j++] = 'Y';		//unitPriceDispPermission
	newData[j++] = '';		//unitPriceDisp
	newData[j++] = '';		//extendedPriceDisp
	newData[j++] = 'Y';		//currencyPermission
	newData[j++] = '' ;    //$v('currency');//currency 
	newData[j++] = '0';		//quantityReceived
	newData[j++] = '';		//quantityVoc
	newData[j++] = '';			//quantityReturned
	newData[j++] = 'Draft';		//status
	newData[j++] = '0';			//amendment
	newData[j++] = 'Draft';		//poLineStatus
	newData[j++] = lineId*1000;			//poLine
	newData[j++] = isMaterialTypeItem;			//isMaterialTypeItem
	newData[j++] = '';			//lastConfirmed
	newData[j++] = '';			//secondarySupplierAddress
	newData[j++] = '';			//itemDescription
	newData[j++] = '';			//mpn
	newData[j++] = '';			//supplierPn
	newData[j++] = '';			//dpas
	newData[j++] = '';			//shelfLife
	newData[j++] = '';			//shelfLifeBasis
	newData[j++] = '';			//supplierDetail
	newData[j++] = '';			//verifiedOn
	newData[j++] = '';			//itemVerifiedBy
	newData[j++] = '';			//lineNotes
	newData[j++] = '';			//deliveryComments
	newData[j++] = 'N';			//everConfirmed
	newData[j++] = 'No';			//validItem
	newData[j++] = '';			//differentSupplierOnLine
	newData[j++] = '';			//secondarySupplierOnPo
	newData[j++] = '';			//changeSupplier
	newData[j++] = '';			//itemUnitPrice
	newData[j++] = '';			//shipFromLocationId
	newData[j++] = '';			//relatedPoLines
	newData[j++] = '';			//prShipTo
	newData[j++] = lineId;			//poLineNumber
	newData[j++] = lineId;			//seqNumber
	newData[j++] = 'Y';			//lineChange
	newData[j++] = '';			//ammendmentcomments
	newData[j++] = '';			//lineArchived
	newData[j++] = canAssignAddCharge;			//canAssignAddCharge
	newData[j++] = '';			//supplierQty
	newData[j++] = '';			//supplierUnitPrice
	newData[j++] = '';			//supplierExtPrice
	newData[j++] = '0';			//lineTotalPrice
	newData[j++] = '';			//unitPrice
	newData[j++] = '';			//extendedPrice
	newData[j++] = '';			//oldNeedDate
	newData[j++] = '';			//oldPromisedDate  
	newData[j++] = '';				//oldProjectedDate
	newData[j++] = '';				//msdsRequestedDate
	newData[j++] = '';				//flowdownChanged
	newData[j++] = '';				//specChanged
	newData[j++] = '';				//lookAheadChanged
	newData[j++] = '';				//chargeListChanged
	newData[j++] = '';				//quantityVouchered
	newData[j++] = '';				//supplier
	newData[j++] = '';				//secondarySupplier	
	newData[j++] = '';				//oldItemId
	newData[j++] = '';				//oldQuantity
	newData[j++] = '';				//oldUnitPriceDisp
	newData[j++] = '';				 //oldCurrency
	newData[j++] = '';				//oldSupplierPn
	newData[j++] = '';				//oldDpas
	newData[j++] = '';				//oldShelfLife
	newData[j++] = '';				//currencyConversionRate
	newData[j++] = '';				//poLineClosed    
    
	var nextId = newId + 1;
    while (lineItemGrid.doesRowExist(nextId)) 
    	nextId++;       
    lineItemGrid.addRow(nextId,newData,lineItemGrid.getRowsNum());
    lineItemGrid.selectRowById(nextId, null, true, true);
	return nextId;
}

function addLineItem(){
	var nextId = addNewItem(getNewLineId(),'MA','Y');
	//createNewDivsForGrids(nextId);
	//selectRow(nextId);
	lineItemGrid.selectRowById(nextId, null, true, true);
	getItemDetail(nextId);
	lineItemGrid.selectRowById(nextId, null, true, true);
}

function addLineCharge() {
	var nextId = addNewItem(getNewLineId(),'','N');
	//createNewDivsForChargeGrids(nextId);
	lineItemGrid.selectRowById(nextId, null, true, true);
	getItemDetail(nextId);
	lineItemGrid.selectRowById(nextId, null, true, true);
}
...
function getNewLineId() {  //from the grid
	...
	var rowsNumId = lineItemGrid.getRowId(lineItemGrid.getRowsNum()-1);  
	var lastnumber = gridCellValue(lineItemGrid,rowsNumId,"line");
	var lineNumber = lastnumber.split("/");
	var lineId = Number(lineNumber[0])+1;
	return lineId;
}
...
function printPo()
{
	...
	if ( (po.value.trim().length > 0) && (validpo.value == "No") ) {
		...
		if (tmp != null)
		{
			if (tmp == "0")
		   {
				if (validateDataForSubmit("printdraft",this))
				{
					//alert("Print PO");
				}
				else
...
function createNewDivsForGrids(rowNumber) {
	//createMainDivElement('materialTypeItem', rowNumber);
	createGridElement('lookAhead', rowNumber, "getLookAheads", "lookaheadimg", "Look Ahead");
	createLineNotesElement('lineNotes',rowNumber,"getCannedComments", "setLineUnconfirmed", "Line Notes"); //Line Notes
	createDeliveryCommentsElement('deliveryComments',rowNumber, "setLineUnconfirmed", "Delivery Notes"); // Deliver Notes
	createGridElement('itemNotes', rowNumber, "getItemNotes", "itemnotesimage", "Item Notes");
	createGridElement('partNotes', rowNumber, "getPartNotes", "partnotesimage", "Part Notes");
	createGridElement('associatedPrs', rowNumber, "getAssociatedPrs", "associatedPrsimage", "Associated PRs");
	createGridElement('specs', rowNumber, "getSpecs", "specimage", "Specs");
	createGridElement('flowdowns', rowNumber, "getFlowdowns", "flowdownimage", "Flow Downs");
	createGridElement('msds', rowNumber, "getMsds", "msdssearchimage", "MSDS");
	createGridElement('tcmBuys', rowNumber, "getTcmBuys", "tcmbuysimage", "TCM Buys");
	createGridElement('clientBuys', rowNumber, "getClientBuys", "clientbuysimage", "Client Buys");	
}

function createMainDivElement(idName, rowNumber) {	
	var div = j$('<div/>', {
	    id: idName+rowNumber,    
	    css: { display:'none' }
	});
	
	var tableElem =  j$('<table/>', { 
		width: '100%', 
		border: '0', 
		cellpadding: '0', 
		cellspacing: '0',
		id: 'tableSearch'+rowNumber
	}).addClass('tableSearch');
	
	j$('#poGrids').append(div.append(tableElem));
}

function createGridElement(divName, rowNumber, methodName, imageName, divLabelName) {
    //Add hidden fields for status and loaded
    var inputStatus = j$("<input/>").attr("type", "hidden").attr("id", divName+"Status"+rowNumber).attr("name", divName+"Status"+rowNumber).attr("value","No");
    var inputLoaded = j$("<input/>").attr("type", "hidden").attr("id", divName+"Loaded"+rowNumber).attr("name", divName+"Loaded"+rowNumber).attr("value","No");
    var label = j$("<label/>").text(divLabelName+":");      //Add label for prompt text
    var anchor = j$('<a/>').attr("style","color: #0000ff; cursor: hand;").attr("href","javascript:"+methodName+"("+rowNumber+")");
    var image = j$('<img/>').attr("id",imageName+rowNumber).attr("alt",divLabelName).attr("src","/images/plus.jpg");    
    var tdElem =  j$('<td/>');
    var trELem = j$('<tr/>', { valign: 'top' });		
	var secondTdElem = j$('<td/>');
	var secondDiv = j$('<div/>', { id: divName+'Bean'+rowNumber });
	var secondImage = j$('<img/>').attr("id","spinner").attr("style","display:none").attr("src","/images/ajax-loader.gif");
	
	//sequence of appending should be maintained for appropriate order of elements
	anchor.append(image);
	tdElem.append(inputStatus).append(inputLoaded).append(label).append(anchor);
	secondTdElem.append(secondDiv).append(secondImage);
	
	j$('#tableSearch'+rowNumber).append(trELem.append(tdElem).append(secondTdElem));	
	j$('#tableSearch'+rowNumber).append('<tr><td colspan="2">&nbsp;</td></tr>');
}

function createLineNotesElement(divName, rowNumber, methodName, secondMethodName, divLabelName) {
	var tdElem =  j$('<td/>');
	var trELem = j$('<tr/>', { valign: 'top' });		
	var label = j$("<label/>").text(divLabelName+":"); 
	var secondTdElem = j$('<td/>');
	var button = j$("<input/>").attr("type","button").attr("class","SUBMIT").attr("value","Canned").attr("onclick", methodName+"('"+rowNumber+"')").attr("onmouseover", "className='SUBMITHOVER'").attr("onmouseout", "className='SUBMIT'");
	var inputTxtArea = j$("<textarea/>").attr("rows", "5").attr("cols", "130").attr("id", divName+rowNumber).attr("name", divName+"Status"+rowNumber).attr("onchange",secondMethodName+"('"+rowNumber+"')");
	
	secondTdElem.append(inputTxtArea);
	tdElem.append(label).append("<br/><c:if test='${readonly == 'N'}'>").append(button).append("</c:if>");
	trELem.append(tdElem).append(secondTdElem);
	j$('#tableSearch'+rowNumber).append(trELem);	
}

function createDeliveryCommentsElement(divName, rowNumber, methodName, divLabelName) {
	var tdElem =  j$('<td/>');
	var trELem = j$('<tr/>', { valign: 'top' });		
	var label = j$("<label/>").text(divLabelName+":"); 
	var inputTxtArea = j$("<textarea/>").attr("rows", "5").attr("cols", "130").attr("id", divName+rowNumber).attr("name", divName+"Status"+rowNumber).attr("onchange",methodName+"('"+rowNumber+"')");
	var secondTdElem = j$('<td/>');
...
function createNewDivsForChargeGrids(rowNumber) {
	createChargeDivElement('addChargesDivBlock', rowNumber);
	createChargeGridElement('chargesList', rowNumber);
}

function createChargeDivElement(idName, rowNumber) {	
	var div = j$('<div/>', {
	    id: idName+rowNumber,    
	    css: { display:'none' }
	});
	...
	div.append(tableElem);
	j$('#addChargesList').append(div);
}

function createChargeGridElement(divName, rowNumber) {	
    //Add hidden fields for status and loaded
    var inputStatus = j$("<input/>").attr("type", "hidden").attr("id", divName+"Status"+rowNumber).attr("name", divName+"Status"+rowNumber).attr("value","No");
    var inputLoaded = j$("<input/>").attr("type", "hidden").attr("id", divName+"Loaded"+rowNumber).attr("name", divName+"Loaded"+rowNumber).attr("value","No");    
    var tdElem =  j$('<td/>', {colspan: '2' });
    var trELem = j$('<tr/>', { valign: 'top' });		
	var innerdiv = j$('<div/>', { id: divName+'Bean'+rowNumber, style: 'width:85%;height:250px;' });
	var secondImage = j$('<img/>').attr("id","spinner").attr("style","display:none").attr("src","/images/ajax-loader.gif");
	
	//sequence of appending should be maintained for appropriate order of elements
	tdElem.append(inputStatus).append(inputLoaded);
	tdElem.append(innerdiv).append(secondImage);
	trELem.append(tdElem);
	j$('#tableSearch'+rowNumber).append(trELem);
}

function getItemDetail(rowId) {	
	if (poLineSelectedRowId == null || poLineSelectedRowId == "") {
		poLineSelectedRowId = 1;
	}
	if (rowId == null || rowId == "")
	...
	//console.log("getItemDetail-poLineSelectedRowId = " + poLineSelectedRowId);
	var shipToId  =  $v("shipToId");
	...
	var mode = "";
	materialType = gridCellValue(lineItemGrid,rowId,"isMaterialTypeItem");
	if ( materialType == 'N' ) {		
		mode = "lineCharge";
	}
	var itemType = 	gridCellValue(lineItemGrid,rowId,"type");	
...
function itemChanged(itemId,itemDesc,itemType) {
	...
	var poLineNumber = poLineSelectedRowId;
...
function resetGridStatuses(rowId) {		
	...
	if (materialType == 'Y') {
		...
		if ($('addChargesList') != null && $('addChargesDivBlock') != null ) {
			...
			selectPoLinesForChargeType(rowId);
			$('lineItemDetailRows').style.display = "none";			
			$('lastconfirmedalt').innerHTML = gridCellValue(lineItemGrid, rowId,'lastConfirmed');
			$('secondarySupplierAddress').innerHTML=gridCellValue(lineItemGrid, rowId,'secondarySupplierAddress');
			$('itemdescalt').innerHTML=gridCellValue(lineItemGrid, rowId,'itemDescription');					
		}
	}
	selectRow(rowId);
}
...
function changeTotalPrice(rowId) {
    ...
    changePoTotalPrice();
    changePoTotalPriceInUSD();  // call this only if the financial approval conditions meet
    setLineUnconfirmed(rowId);
}
...
{	
    ...
    //{
	  ...
	  //var lineunitprice = gridCellValue(lineItemGrid,j,'unitPrice');      
	  //totalpoPrice += ( (lineqty*1) * (lineunitprice*1) );	  
    //}
    //$("totalCost").value = totalpoPrice.toFixed(4);
    //$("totalCostSpan").innerHTML = totalpoPrice.toFixed(4);
    
    var currencySelected = new Array();
    currencySelected = getCurrency();
    //alert(currencySelected);
    var totalCurrPrice = new Array();
    totalCurrPrice = getTotalCurrPrice(currencySelected);
    //alert(totalCurrPrice);
    var strTotalCostSpan = "";
    var val = 0;
    for(j=0; j<currencySelected.length; j++)
    {
    	val = totalCurrPrice[j]*1;
    	if(j>0)
    		strTotalCostSpan = strTotalCostSpan + "</br>";
    	strTotalCostSpan += val.toFixed(4) + " " + currencySelected[j];    	
    }
    $("totalCostSpan").innerHTML = strTotalCostSpan;
...
function getTotalCurrPrice(currencySelected) {
	var totPriceForCurrSelected = new Array();
	var nowofRows = (lineItemGrid.getRowsNum())*1;
	var ifound = 0;
	for(cnt = 0; cnt < currencySelected.length; cnt++) {
		var arrCurrency = currencySelected[cnt];
		...
		for(j = 1; j <= nowofRows; j++)
	    {
			...
			var lineunitprice = gridCellValue(lineItemGrid,j,'unitPrice');
			if (currency == arrCurrency) {
				currLineTotalPrice += ( (lineqty*1) * (lineunitprice*1) );	
				totPriceForCurrSelected[cnt] = currLineTotalPrice;	
			}					
	    }
	}
	
	return totPriceForCurrSelected;
}
...
function secondarySupplierPNChanged() {
	if (poLineSelectedRowId == null || poLineSelectedRowId == "") {
		poLineSelectedRowId = 1;
	}
	lineItemGrid.cellById(poLineSelectedRowId, lineItemGrid.getColIndexById("supplierPn")).setValue($v("supplierPn"));
	lineItemGrid.cellById(poLineSelectedRowId, lineItemGrid.getColIndexById("dpas")).setValue($v("dpas"));
	setLineUnconfirmed(poLineSelectedRowId);
}
...
function currencyChanged() {
	...
	for(rowId = 1; rowId <= noOfRows; rowId++)
    {
		var linestatus = gridCellValue(lineItemGrid,rowId,"status");
		var canassignaddcharge = gridCellValue(lineItemGrid,rowId,"canAssignAddCharge");
		if ( (linestatus != "Remove") )
		{
			setLineUnconfirmed(rowId);
			changeTotalPrice(rowId);
			if (canassignaddcharge == "Y")
				changeSuppTotalPrice(rowId);
		}
    }
}
*/
	...
	j$.ajax({
		 ...
		 data: {radianPo: $v("radianPo"),
				hubName: $v("hubName"),		 		
		 		...
		 		shipToLocationId: $v("shipToId"),
		 		customerPo: $v("customerPo"),
		 		dateSent: $v("faxdate"),
		 		dateAccepted: $v("acceptedDate"),
		 		suppRating: $v("suppRating"),
		 		invenGrp: $v("invenGrp"),
		 		consignedPo: varConsignedPo,
		 		orderTakerID: $v("orderTakerID")
		 },
...
function saveLookaheadInfo(rowNumber) {
	...
	$("rowNumber").value = gridCellValue(lineItemGrid,poLineSelectedRowId,"poLineNumber");
	
	var frmdt =  j$("form").serialize(); // serializes the form's elements.
...
function lookaheadSaveComplete(msg) {
	...
	clearGridForms();
	 ...
	 else
	 {
		 ...
		 getLookAheads();
		 lineItemGrid.cellById(previousRowId, lineItemGrid.getColIndexById("lookAheadChanged")).setValue("N");
		 alert("Look Ahead information saved successfully for PO line " + poLineSelectedRowId);
	 }	 
}
...
function savePoLineChanges(rowNumber) {
	...
	$("rowNumber").value = gridCellValue(lineItemGrid,rowNumber,"poLineNumber");
...
function savePoLineChangesComplete(msg) {
	...
	if(msg != "OK") {
		...
		alert("PO Line information saved successfully for line " + poLineNumberSaved);
	}
	...
	if ($v("action") == "deletePoLine" || type == "N" || status == "Draft")
		reloadPoLine();
}
...
function msdsSaveComplete(msg) {
	...
	if (poLineSelectedRowId == null || poLineSelectedRowId == "") {
	 ...
	 {
		 if (j$("#sendMsdsCheckBox").prop('checked'))		 
		 ...
		 alert("MSDS information saved successfully for line " + poLineSelectedRowId);
...
function specsSaveComplete(msg) {
	...
	if (poLineSelectedRowId == null || poLineSelectedRowId == "") {
	 ...
	 if(msg != "OK") {
		 ...
		 lineItemGrid.cellById(poLineSelectedRowId, lineItemGrid.getColIndexById("lineChange")).setValue("N"); //TODO:Check if we need this?
		 alert("Specs information saved successfully for line " + poLineSelectedRowId);
	 }
	 clearGridForms();
}
...
function flowdownsSaveComplete(msg) {
	...
	if (poLineSelectedRowId == null || poLineSelectedRowId == "") {
	 ...
	 if(msg != "OK") {
         ...
         lineItemGrid.cellById(poLineSelectedRowId, lineItemGrid.getColIndexById("lineChange")).setValue("N"); //TODO:Check if we need this?
		 alert("Flowdowns information saved successfully for line " + poLineSelectedRowId);
	 }
	 clearGridForms();
}
...
function financialApprovalComplete(msg) {
	...
	clearGridForms();
	 if(msg != "OK") {
		 ...
		 document.getElementById("confirmlink").style["display"] = "";
...
function reloadPoLine() {
	showTransitWin("");
	callToServer("purchaseorder.do?action=reloadpoline&po="+$v("po")+"&radianPo="+$v("po"));	
}
...
//	var elements = document.getElementById("genericForm").elements;
//	for (var i = 0, element; element = elements[i++];) {
//	    if (element.type === "hidden" )
//	        console.log("Before element.submitName = " + element.name);
//	}
	//alert(lineItemGrid.formInputs);
...
//	for (var i = 0, element; element = elements[i++];) {
//	    if (element.type === "hidden" )
//	        console.log("After element.submitName = " + element.name);
//	}
}

/* onChange event for a select box
function setSelectFlag(rowId, colId) {
	if (poLineSelectedRowId == null || poLineSelectedRowId == "") {
		poLineSelectedRowId = 1;
	}
	var gridNum = poLineSelectedRowId;
	
	alert(gridCellValue(window['chargesListGrid'+gridNum],rowId,"selectPo") +" and "+gridNum+" and " +rowId);	
	//window['chargesListGrid'+gridNum].cellById(pos, window['chargesListGrid'+gridNum].getColIndexById("selectPo")).setValue(true);	
}


function poLineChangesFound() {
	if (poLineChangesMade) {
		var response = confirm("You have made some changes to Po Line " + previousRowId + ". Would you like to save your changes? If you select Cancel then your changes will be lost.")
		if (response == true) { // pressed OK
			poLineChangesMade = false;
			savePoLineChanges(previousRowId);
		} else {// pressed Cancel
			poLineChangesMade = false;
		}
...
function getBuyOrders() {
	...
	else if (invenGrp.value.trim().length == 0 || opsEntityId.value.trim().length == 0)
	{
	    alert("Please choose a valid Ship To.");
	}
	...
	{
		loc = "/tcmIS/supply/editassociatedprsmain.do?hubName="+hubName.value;
	    ...
	    if (radianPO.value.length > 0)
	    {
	        ...
	        if ((shipToId.length > 0) && (validShipTo.value == "Yes")) {
	           ...
	           loc = loc  + "&shipToLocationId=";
	        }
	        loc = loc + "&shipToCompanyId=" + shiptoCompanyId.value;
	        loc = loc + "&buyerId=" + buyerId.value;
...
function fobChanged()
{
	$("fob").value = $("selectFob").value;
}

function paymentTermsChanged()
{
	$("paymentTerms").value = $("selectPaymentTerms").value;
}
