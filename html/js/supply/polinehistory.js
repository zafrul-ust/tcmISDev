...
{  
 ...
 if (totalLines > 0)
 {
  document.getElementById("poLineHistoryViewBean").style["display"] = ""; 
  /*this is the new part.*/
  doInitGrid();  
 }
 else
 {
   document.getElementById("poLineHistoryViewBean").style["display"] = "none";   
 }
...
mygrid = new dhtmlXGridObject('poLineHistoryViewBean');
...
mygrid.setHeader(""+messagesData.amendment+","+messagesData.itemid+","+
		     	 messagesData.quantity+","+messagesData.unitprice+","+
		     	 messagesData.needed+","+messagesData.promiseddate+","+
		     	 messagesData.projecteddelivery+","+messagesData.dateconfirmed+","+		     	 
		     	 messagesData.notes+","+messagesData.amendmentremarks);
...
mygrid.setInitWidths("5,5,5,7,7,7,7,7,30,30");

/*You can set column alingments, all string and date values will be left aligned and numbers will be right aligned.*/
mygrid.setColAlign("right,right,right,left,left,left,left,left,left,left");
...
mygrid.setColSorting("int,int,int,haasStr,int,int,int,int,haasStr,haasStr");
...
**/
mygrid.enableTooltips("false,false,false,false,false,false,false,false,true,true");
...
mygrid.setColumnIds("amendment,itemId,quantity,unitPrice,needDate,promisedShipDate,projectDeliveryDate,dateConfirmed,notes,remarks");
...
*/
mygrid.setColumnsVisibility("false,false,false,false,false,false,false,false,false,false");
...
function sortValues(ind,type,direction)
{
	...
	switch (columnId)
 	{
 	case "itemId":
 		colIndex=mygrid.getColIndexById("itemId");
 		break; 
 	case "quantity":
 		colIndex=mygrid.getColIndexById("quantity");
 		break; 
 	case "needDate":
 		colIndex=mygrid.getColIndexById("needDate");
 		break; 
 	case "promisedShipDate":
 		colIndex=mygrid.getColIndexById("promisedShipDate");
 		break;
 	case "projectDeliveryDate":
 		colIndex=mygrid.getColIndexById("projectDeliveryDate"); 
 		break;
 	case "dateConfirmed":
