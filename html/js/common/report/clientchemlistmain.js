...
function facilityChanged() {
    ...
    showPurchasingMethod(selectedFacility);

    if('Y' == permissionArr[$v('facilityId')])
		$("uploadB").style.display="";
	else
		$("uploadB").style.display="none";
}

function showPurchasingMethod(selectedFacility) {
    var purchasingMethod = $("source");
    //clear data
    for ( var i = purchasingMethod.length; i > 0; i--) {
		purchasingMethod[i] = null;
	}
    //set new data
    if (purchasingMethodArr != null) {
        setOption(0, messagesData.all, "", "source");
        var dataCount = 1;
        for (var i = 0; i < purchasingMethodArr.length; i++ ) {
            if (purchasingMethodArr[i].facilityId == selectedFacility)
                setOption(dataCount++, purchasingMethodArr[i].purchasingMethodName, purchasingMethodArr[i].purchasingMethodId, "source");
        }
    }else {
        setOption(0, messagesData.all, "", "source")
    }
...
function workAreaChanged(index) {
	$("workAreaName").value = $("workArea").options[index].text;
}
