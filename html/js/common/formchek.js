...
// isFixLenDegit (s,len, [,eok])       True if string s is digit (0-9) of a length.
// isFixLenAlphanumeric (s,len, [,eok])True if string s is English letters and numbers (0-9) of a length.
//
...
/*Nawaz 04-03-07 this was causing problems with the menu*/
...
function checkPasswords(entered)
{
		var result ;
      var finalMsgt;
      var allClear = 0;
    ...
    var oldpasswd  =  document.getElementById("oldpasswd");
	 ...
	 if (oldpasswd.value.trim().length > 0 )
	 {
	 ...
	 {
		allClear = 1;
		finalMsgt = finalMsgt + " Please Enter Your Old Password.\n\n" ;
	 }

    if (newpasswd1.value.trim().length > 0 && newpasswd1.value.trim().length > 0 && newpasswd1.value.trim() == newpasswd2.value.trim() )
    {
    ...
    {
	   allClear = 1;
		finalMsgt = finalMsgt + " Please Enter a Valid Password and Make Sure you Type it Same Twice.\n\nThanks." ;
    }
...
function isFixLenDegit (s,len, eok) {
...
function isFixLenAlphanumeric (s,len, eok) {
    var emptyOK = defaultEmptyOK;
    if ( arguments.length > 2 )
    	emptyOK = eok;
    if( isEmpty(s) ) return emptyOK;
    if( s.length != len ) return false;
    for( var i = 0 ;i < len ; i++ ) 
    	if( !isLetterOrDigit ( s.charAt(i) ) ) return false;
    return true;
}
// checkzip.
// USA and BRA are the only ones now.
var zipPlusCountry = { 'USA':4,'BRA':3,'PRI':4,'MEX':4 };
function setZipPlus(country,spanid,zipplusid) {
  if( typeof( country ) == 'string' ) 
  	country = $(country);
  try {
  if( zipPlusCountry[ country.value ] != undefined ) 
  	$(spanid).style.display="";
  else
  	$(spanid).style.display="none";
  	if(zipplusid)$(zipplusid).value="";
  }
  catch(ex){}
}

function zipCheck ( country,zip,zipplus ) 
{
	var zips = zip.split(/-/);
	if( zips.length > 1 && zips[1].trim() != '' ) {
		zip = zips[0].trim();
		zipplus = zips[1].trim();
	}
// try to make country that more frequently appeared in front, to make it run faaster.   
	if( country == 'USA' || country == 'PRI' || country == 'MEX') {
//		return zip.match(/\d{5}/) && ( !zipplus || zipplus.match(/\d{4}/);
		return isFixLenDegit(zip,5,false) && isFixLenDegit(zipplus,4,true);
	}    
	else if( country == 'CAN' ) {
		return zip.match(/[a-zA-Z0-9]{3} [a-zA-Z0-9]{3}/);
	}
	else if( country == 'GBR' ) {
		return zip.match(/^([A-PR-UWYZ0-9][A-HK-Y0-9][AEHMNPRTVXY0-9]?[ABEHMNPRVWXY0-9]? {1,2}[0-9][ABD-HJLN-UW-Z]{2}|GIR 0AA)$/);
	}
    else if( country == 'DEU' || country == 'ISR' || country == 'ITA' ||country == 'FRA' || country == 'POL' || country == 'MYS' ||country == 'TUR') {
		return isFixLenDegit(zip,5,false);
	}
    else if( country == 'SGP' || country == 'CHN') {
		return isFixLenDegit(zip,6,false);
	}      
    else if( country == 'BRA' ) {
		return isFixLenDegit(zip,5,false) && isFixLenDegit(zipplus,3,true);
	}      
	
	return true;
}

// Format an I18N message by replacing marked variable values
function formatMessage (message) {
	var resultString = message;
	var value;
	var key;

	// Loop through all arguments after message replacing {0}, {1}, etc.
	for (var i = 1; i < formatMessage.arguments.length; i++) {
		value = formatMessage.arguments[i];
		key = "{" + (i * 1 - 1 ) + "}";
