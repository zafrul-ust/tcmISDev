...
function validateForm() {
 ...
 // This validation is for simple search   
    ...
    if ($v("hideOrShowDiv") == "show" && isWhitespace($v("searchText")) && $v("physicalState").length == 0 && $v("mfgId").length == 0 
    	&& isWhitespace($v("ph")) && isWhitespace($v("flashPoint")) && isWhitespace($v("vaporPressure"))
        && isWhitespace($v("vocPercent")) && isWhitespace($v("vocLwesPercent")) && isWhitespace($v("solidsPercent"))
    	...
    	&& (($('list').checked && listHasInputFlag == false) || ($('casNos').checked && casHasInputFlag == false))
    	&& workAreaFlag == false && $("stocked").checked == false && $("kitOnly").checked == false
        && ($('fullDatabase').checked || $v('facilityId') == '' || $("approvedOnlyDisplay").checked) )
    {
        if (specificErrorMsg.length == 0) {
        	if($('fullDatabase').checked || $("approvedOnlyDisplay").checked)
        		specificErrorMsg = messagesData.searchTextOnly;
	        else
	            specificErrorMsg = messagesData.searchTextRequired;
        }else {
        	if($('fullDatabase').checked || $("approvedOnlyDisplay").checked)
        		specificErrorMsg += "\n"+ messagesData.searchTextOnly;
	        else
...
function submitSearchForm() {
    ...
    //showPleaseWait();
	children[children.length] = openWinGeneric('/tcmIS/common/loadingpleasewait.jsp', "advancedmsdsviewerresults", "1024", "720", "yes", "50", "50", "20", "20", "no");
	//openWinGeneric('/blank.html',"advancedmsdsviewerresults","650","600","yes");  
	 //openWinGeneric(loc, "advancedmsdsviewerresults", "1024", "720", "yes", "50", "50", "20", "20", "no");
    document.msdsViewerForm.target='advancedmsdsviewerresults';
...
{	
    ...
    for (var i=0; i<application.options.length; i++) {
	    if (application.options[i].selected) {
	      applicationList += ";" + application.options[i].value;
	      applicationDesc += "&@#" + application.options[i].text;
	    }
	}
    
    $("applicationDesc").value = applicationDesc;
	
	$("applicationList").value = applicationList.substring(1);
	if($v("applicationList").substring(0,1) == ";") 
	 ...
	 $("facilityName").value = facilityId.options[facilityId.selectedIndex].text;

     document.msdsViewerForm.target='MSDSExcel';
     //openWinGenericViewFile('/tcmIS/common/loadingfile.jsp',"MSDSExcel","650","600","yes");
     openWinGenericExcel('/tcmIS/common/loadingfile.jsp',"MSDSExcel","650","600","yes");
     window.setTimeout("document.msdsViewerForm.submit();",300);
}
