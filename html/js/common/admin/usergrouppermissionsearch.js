...
function createNewUserGroup(){
    ...
    if (flag) {
        ...
        var loc = "createnewusergroup.do?userGroupType="+$v("userGroupType")+"&companyId="+companyId.value+"&facilityId="+facilityId.value+"&facilityName="+facilityName;
...
function facilityChanged() {
  ...
  for(var i=0; i < facilityPermissionArray.length; i++) {
    if(facilityPermissionArray[i] == facilityId.value) {
      found = true;
      break;
    }
  }
  ...
  if(found) {
    document.genericForm.createNewGroup.style.display = "";
  }else {
    document.genericForm.createNewGroup.style.display = "none";
  }
}
