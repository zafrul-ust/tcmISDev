...
function submitUpdate() {
	 if (window.frames["resultFrame"].preUpdateAudit()) {
...
function createXls () {
	window.frames["resultFrame"].document.getElementById("action").value='createXSL';
	window.frames["resultFrame"].document.genericForm.target='_edit_list_excel_report_file';
	openWinGenericExcel('/tcmIS/common/loadingfile.jsp','_edit_list_excel_report_file','650','600','yes');
	//setTimeout("window.frames[resultFrame].document.genericForm.submit()",300);
	window.frames["resultFrame"].document.genericForm.submit();
}