...
function submitUpdate() {
	...
	var action = window.frames["resultFrame"].document.getElementById("uAction");
	action.value = 'update';
	showPleaseWait();
	var now = new Date();
	...
	window.frames["resultFrame"].beangrid.parentFormOnSubmit();
...
function submitAdd() {
	...
	if (selectedIndex < 0) {
		...
		window.frames["resultFrame"].document.getElementById("listId").value = obj.listId;
		window.frames["resultFrame"].document.getElementById("approver").value = "checked";
		showPleaseWait();
		/*Submit the form in the result frame*/
		window.frames["resultFrame"].document.genericForm.submit();
	}
