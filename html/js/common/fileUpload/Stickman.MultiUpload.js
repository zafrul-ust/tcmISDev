...
var MultiUpload = new Class(
{
	...
	initialize:function( input_element, max, name_suffix_template, show_filename_only, remove_empty_element ){
		...
		if( this.remove_empty_element){
			try {
				input_element.form.addEvent('submit',function(){this.elements.getLast().element.disabled = true;});
			}
			catch (Ex) {}
		}
	},
	...
	addRow:function(){
		if( this.max == 0 || this.elements.length <= this.max ){
			...
			alert( formatMessage(messagesData.maxUpload, this.max));
		}
	...
	deleteRow:function( uid ){
		...
		if( confirm(formatMessage(messagesData.verifyRemove, deleted_row.element.value)) ){
