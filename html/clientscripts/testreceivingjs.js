...
/*function createxls()
{
   ...
   alert(cookieArray);
   biscuit=new siteCookie( cookieArray );
}
...
{
//alert("Sending values to the mani window");
...
function getReceiptnotes(receiptId)
{
    ...
    if (receiptId.length > 0)
    {
       ...
       openWinGeneric(loc,"ShipToID","550","200","yes")
    }
}
...
function splitQty(receiptid,Hub,Qtyava)
{
    ...
    openWinGeneric(loc,"Previous_Transactions","300","250","no");
}
...
function addnewBin()
{
  ...
  openWinGeneric(newbinURL,"add_newbin","300","150","no");
}
...
function showPreviousReceiptTransactions(receiptid,Hub)
{
    ...
    loc = loc + receiptid;
    loc = loc + "&HubNo=" + Hub;
    openWinGeneric(loc,"Previous_Transactions","650","400","no");
}
...
function showPreviousrecqc(poline,Hub)
{
    ...
    loc = loc + poline;
    loc = loc + "&HubNo=" + Hub;
    openWinGeneric(loc,"Previous_Receiving","600","300","no");
}
...
function showReceivingJacket(poline,itemID)
{
...
function doreloadpage()
{
	 ...
	 for ( var p = 0 ; p < 1000 ; p ++)
    {
		for ( var p = 0 ; p < 1000 ; p ++)
    	{
  			now2 = new Date();
	      //alert('difference = ' + (Date.parse(now2) - Date.parse(now1)));
	      if ( (Date.parse(now2) - Date.parse(now1)) > 11000)
	      {
	      break;
	      }
	   }
	 ...
	 //alert("here");
	 doyou = confirm("Do you like to Post Again?");
    if (doyou == true)
    {
    ...
    window.document.receiving.submit();
    return true;
    }
	 else
	 {
...
function checkInvenValues(name ,entered)
{
    ...
    for ( var p = 0 ; p < total ; p ++)
    {
		...
		eval( testrec = "window.document.receiving.recerts" + line_num.toString() )
            ...
            if ( recert*1 < 0 )
            {
		    //finalMsgt = finalMsgt + " No of Recerts. \n";
                //allClear = 1;
            ...
            var expDate = eval(testexpirydate.toString()).value;
            if ( (expDate == "INDEFINITE") || (expDate == "Indefinite")  )
            {
            }
            else
            {
...
function checkValuesqc(name ,entered)
{
    ...
    for ( var p = 0 ; p < total ; p ++)
    {
		...
		finalMsgt = "Please enter valid values for: \n\n";
            ...
            {
                finalMsgt = finalMsgt +   " Lot Status can not be Incoming. \n" ;
                allClear = 1;
            }
            ...
            eval( testexpirydate = "window.document.receiving.expiry_date" + line_num.toString() )
            if (!((eval(testexpirydate.toString())).value == "Indefinite" ))
            {
                if ( (eval(testexpirydate.toString())).value.length == 10 )
                {
                    if ( checkexpirydate((eval(testexpirydate.toString()))) == false )
...
function doUnConfPopup()
{
  ...
  openWinGeneric(unconfURL,"Generate_UNCONF_labels","640","600","no")
}
...
function doPopup()
{
   ...
   ( curval =  (eval(testbin.toString())).options[cur].value );
    ...
    openWinGeneric(testurl3,"Generate_labels","640","600","no")
}
