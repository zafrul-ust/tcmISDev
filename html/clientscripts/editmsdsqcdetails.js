...
function actionValue(name, entered)
{
    ...
    if (actvalue == "Save")
    {
	    ...
	    eval(flashpointO  =  "window.document.ComponentForm" + (p+1).toString() + ".flashpoint" +p.toString() );
            ...
            {
                finalMsgt = finalMsgt +   " Flash Point Unit.\n" ;
                allClear = 1;
            }
    ...
    for ( var p = 0 ; p < total ; p ++)
    {
        ...
        if ( !(isSignedFloat(curval31)) ){ curval31 = "";}
...
function ShowOriginal(entered)
{
    with (entered)
    {
        ...
        loc = loc + "&Company=" + window.document.MainForm.CompanyId.value;
        openWinGeneric(loc,"Original","600","350","yes")
    }
}
...
if (company == "SEAGATE")
        ...
        {
            loc = "/tcmIS/sd/ViewMsds?cl=Boeing&showspec=N&act=msds&id="
                loc = loc + matid;
        }
	  else
	  {
