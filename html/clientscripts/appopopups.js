...
function searchbillingAddressWithUpdate()
{
    ...
    else
    {
        ...
        openWinGeneric(loc,"editbillingAddress","840","420","yes","40","80")
    }
}
...
function sendSupplierWithUpdate(entered)
{
    with (entered)
    {
        ...
        if ( (eval(testmfgid.toString())).value.length > 0 && sapVendorCode.length > 0 && locationStatus != 'Rejected' )
        {
            ...
            if (eval(testmfgid.toString()).value.length == 0)
               alert("Please select a Billing Location.");
            else
                alert("You selected an unapproved Billing Location.  Please select another.");
        }
    }
}
...
var supplierId = "";
var billingLocationId = "";
var locationStatus = "";
var sapVendorCode = "";

function selectedSupplierLocation(rowSupplierId,matidvalue,matdesc,addline1,addline2,addline3,rowLocationStatus,rowSapVendorCode)
{
    document.SupplierLike.supplierid.value = matidvalue;
    document.SupplierLike.supplierdesc.value = matdesc;
    document.SupplierLike.addline1.value = addline1;
    document.SupplierLike.addline2.value = addline2;
    document.SupplierLike.addline3.value = addline3;
    billingLocationId  = matidvalue;
    supplierId = rowSupplierId;
    locationStatus = rowLocationStatus;
    sapVendorCode = rowSapVendorCode;
    setUpdateSupplierLocationAction();
}

function setUpdateSupplierLocationAction() {
    if (locationStatus == '' || locationStatus == 'Inactive') {
        document.getElementById('activateLocation').style["display"]="";
        document.getElementById('inactivateLocation').style["display"]="none";
    }else if (locationStatus == 'Approved') {
        document.getElementById('activateLocation').style["display"]="none";
        document.getElementById('inactivateLocation').style["display"]="";
    }else {
        document.getElementById('activateLocation').style["display"]="none";
        document.getElementById('inactivateLocation').style["display"]="none";
    }
...
function updateSupplierBillingLocation(locationStatus)
{
    var supplierrep = supplierId.replace(/&/gi, "%26");
    var billingLocationIdRep = billingLocationId.replace(/&/gi, "%26");
    //update supplier billing location
    loc = "/tcmIS/Invoice/newinvoice?Action=updateSupplierBillingLocation&supplier="+supplierrep+"&locationStatus="+locationStatus+"&sapVendorCode="+sapVendorCode;
    loc = loc + "&supplierBillingLocationId="+billingLocationIdRep+"&withUpdate="+document.getElementById("withUpdate").value;
    window.location = loc;
}

function showLocation(locationStatus)
{
    var supplierrep = document.getElementById("suppID").value.replace(/&/gi, "%26");
    loc = "/tcmIS/Invoice/newinvoice?Action=showLocation&suppID="+supplierrep+"&locationStatus="+locationStatus+"&sapVendorCode="+sapVendorCode;
    loc = loc + "&withUpdate="+document.getElementById("withUpdate").value;
    window.location = loc;
}
