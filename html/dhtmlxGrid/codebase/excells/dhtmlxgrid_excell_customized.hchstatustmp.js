...
hcalMonth["de_DE"] = {"Jan":0,"Feb":1,"Mär":2,"Apr":3,"Mai":4,"Jun":5,"Jul":6,"Aug":7,"Sep":8,"Okt":9,"Nov":10,"Dez":11};
	 ...
	 this.setValue=function(val){
		...
		if (permission == 'Y') {	
		    ...
		    if (maxlength == 0) {
		    	...
		    	this.setCValue("<input class='inputBox' type='text' id='" + textId +  "' value='" + val + "'"+hedOnChange+" size='" + size + "' maxlength='" + maxlength + "'/>",val);
		    }
		} else {
			if (val.length == 0) {
				this.setCValue("<label>&nbsp;</label>",val);
...
function eXcell_hchstatus(cell){                                    
	 ...
	 this.setValue=function(val){
	    ...
	    var chkboxId="" + columnId+ this.cell.parentNode.idd;
	    if (val == true || val == 'true' || val == 'Y') {
			if (permission == 'Y') {
				this.setCValue("<input type='checkbox' id='" + chkboxId +  "' value='Y' checked onclick=\"updateHchStatusA('" +  chkboxId + "',"+this.cell.parentNode.idd+",'"+columnId+"');\"/>");
			} else {
				this.setCValue("<input type='checkbox' id='" + chkboxId +  "' value='Y' checked disabled />");
			}
	    } else {
	    	if (permission == 'Y') {
	    		this.setCValue("<input type='checkbox' id='" + chkboxId +  "' value='Y' onclick=\"updateHchStatusA('" +  chkboxId + "',"+this.cell.parentNode.idd+",'"+columnId+"');\"/>");
			} else {
				this.setCValue("<input type='checkbox' id='" + chkboxId +  "' value='Y' disabled/>");
			}	   
	    } 	
	 };
	 this.getValue=function(){ 
