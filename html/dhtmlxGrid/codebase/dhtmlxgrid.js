	...
	*/
	this._doContClick=function(ev){ 
		...
		if (ev.button == 2||(_isMacOS&&ev.ctrlKey)){
			// Right-Click!! so select the row
			if (this._haas_debug)
				debug("Right click, doing select of row == " + el.parentNode.idd);
			this.selectCell(this.getRowIndex(el.parentNode.idd), el._cellIndex);
...
dhtmlXGridObject.prototype={
	...
	*/
	...
	doClick:function(el, fl, selMethod, show){
		if (!this.selMultiRows) selMethod=0; //block programmatical multiselecton if mode not enabled explitly
		var psid = this.row ? this.row.idd : 0;
...
//						]);
//				}, 100);
					this.callEvent("onRowSelect", [rid, cid._cellIndex]);
			} else this.callEvent("onRowSelectRSOnly",[rid]);
		...
		if (this.formInputs)
		{
			...
			for (var i = 0; i < this.formInputs.length; i++)
			{
				if(useRemoveChild)
					...
					this.formInputs[i].parentNode.removeChild(this.formInputs[i]);
			}
		}
