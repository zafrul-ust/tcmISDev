...
dhtmlXGridObject.prototype.attachHeader = function () {
	...
	if (this._realfake)
		return !0;
	this.formAutoSubmit();
	typeof this.FormSubmitOnlyChanged == "undefined" && this.submitOnlyChanged(!0);
	typeof this._submitAR == "undefined" && this.submitAddedRows(!0);
	var a = this;
	this._added_rows = [];
	this._deleted_rows = [];
	this.attachEvent("onRowAdded", function (b) {
		a._added_rows.push(b);
		a.forEachCell(b, function (a) {
			a.cell.wasChanged = !0
		});
		return !0
	});
...
dhtmlXGridObject.prototype.createFormInput = function (a, b) {
	...
	c.type = "hidden";
	c.name = this._input_mask && typeof a != "string" ? this._input_mask.apply(this, a) : ((this.globalBox || this.entBox).id || "dhtmlXGrid") + "_" + a;
	c.value = b;
	this.parentForm.appendChild(c);
	this.formInputs.push(c)
};
dhtmlXGridObject.prototype.createFormInputRow = function (a) {
	for (var b = (this.globalBox || this.entBox).id, c = 0; c < this._cCount; c++) {
		...
		if ((!this.FormSubmitOnlyChanged || d.wasChanged()) && (!this._submit_cols || this._submit_cols[c]))
			this.createFormInput(this._input_mask ? [b, a.idd, c] : a.idd + "_" + c, d.getValue())
	}
};
dhtmlXGridObject.prototype.formCreateInputCollection = function () {
	...
	if (this.FormSubmitSerialization)
		...
		this._submitAR &&
		(this._added_rows.length && this.createFormInput("rowsadded", this._added_rows.join(",")), this._deleted_rows.length && this.createFormInput("rowsdeleted", this._deleted_rows.join(","))), this.forEachRow(function (a) {
			this.getRowById(a);
			this.createFormInputRow(this.rowsAr[a])
		})
};
