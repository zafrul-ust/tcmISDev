...
*/
var evenoddmap = null;
dhtmlXGridObject.prototype.setEvenoddmap = function (map) {
	evenoddmap = map;
}
...
*   @topic: 0
*/
dhtmlXGridObject.prototype.enableSmartRenderingRowSpan=function(mode,map,buffer,reserved){evenoddmap=map;this.enableSmartRendering(mode,buffer,reserved);};
...
dhtmlXGridObject.prototype._add_from_buffer=function(ind){
	    ...
	    if (!this._cssSP){
		    if (this._cssEven && (evenoddmap?evenoddmap[ind]:ind)%2 == 0 )
				row.className=this._cssEven+((row.className.indexOf("rowselected") != -1)?" rowselected ":" ")+(row._css||"");
			else if (this._cssUnEven && (evenoddmap?evenoddmap[ind]:ind)%2 == 1 )
...
//		var rh = f[2].firstChild.style.height || this._srdh;
		var rh = f[2].firstChild.style.height.split(/px/)[0] || this._srdh;
		var rr = 0;
		try{ rr = parseFloat(rh); } catch(ex){};
//		alert(f[2].firstChild.style.height+":"+this._srdh+"::"+rh);
		var fr = 0;
		try{ fr = rr+right*this._srdh; } catch(ex){};
		if( fr < 22 ) fr = 22;
		f[2].firstChild.style.height=fr+"px";	
//		f[2].firstChild.style.height=parseFloat(f[2].firstChild.style.height.split(/px/)[0])+right*this._srdh+"px";	
	}
	}
