...
<script type="text/javascript" src="/js/hub/customerreturnsmain.js"></script>
...
<title>
<fmt:message key="customerreturns.title"/>
</title>
...
-->
<body bgcolor="#ffffff"  onload="loadLayoutWin('','customerreturns');setOps();" onresize="resizeFrames()">
 ...
 <div class="roundright">
   ...
   <div class="roundContent">
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableSearch">
      				<tr>
				      <td nowrap  width="5%" class="optionTitleBoldRight" nowrap><fmt:message key="label.operatingentity"/>:</td>
				      <td width="30%" class="optionTitleBoldLeft">
				         <select name="opsEntityId" id="opsEntityId" class="selectBox">
				         </select>
				      </td>
        <td width="25%" class="optionTitleBoldRight" nowrap="nowrap"><fmt:message key="label.receiptid"/>:</td>
        <td width="30%">
           <input name="receiptId" id="receiptId" type="text" class="inputBox" value='<c:out value="${param.receiptId}"/>' size="20" onkeypress="onKeyPress(event)"/>
        </td>
				</tr>
      <tr>
        <td nowrap  width="5%" class="optionTitleBoldRight"><fmt:message key="label.hub"/>:</td>
        <td width="30%">
          <select name="hub" id="hub" class="selectBox" />
		</td>
        <td width="15%" class="optionTitleBoldRight"><fmt:message key="label.mr"/>:</td>
        <td width="30%">
          <input name="mrNumber" id="mrNumber" type="text" class="inputBox" value='<c:out value="${param.mrNumber}"/>' size="20" onkeypress="onKeyPress(event)"/>
        </td>
        <%-- <td width="10%">&nbsp;</td>
        <td width="10%">&nbsp;</td> --%>
      </tr>
      <tr>
        <td nowrap  width="5%" class="optionTitleBoldRight"><fmt:message key="label.inventorygroup"/>:</td>
        <td width="30%">
        ...
        <td width="15%" class="optionTitleBoldRight"><fmt:message key="label.part"/>:</td>
        <td width="30%">
          <input name="catPartNo" id="catPartNo" type="text" class="inputBox" value='<c:out value="${param.catPartNo}"/>' size="20" onkeypress="onKeyPress(event)"/>
        </td>
        <%-- <td width="10%">&nbsp;</td>
        <td width="10%">&nbsp;</td> --%>
      </tr>
      <tr>
        <td nowrap  width="5%" class="optionTitleBoldRight"><fmt:message key="label.facility"/>:</td>
        <td width="30%">
          <select name="facilityId" id="facilityId" class="selectBox"/>
        </td>
        <td width="15%" class="optionTitleBoldRight"><fmt:message key="label.item"/>:</td>
        <td width="30%">
          <input name="itemId" id="itemId" type="text" class="inputBox" value='<c:out value="${param.itemId}"/>' size="20" onkeypress="onKeyPress(event)"/>
        </td>
     </tr>
     
      <tr>
        <td nowrap width="5%" class="optionTitleBoldRight"><fmt:message key="label.returnfor"/>:</td>
        <td width="30%" class="optionTitleBoldLeft">
<input type="radio" class="radioBtns" name="returnForCredit" value="N" checked="checked" id="returnForCredit1" onclick="checksearch()"/>
<fmt:message key="label.customerowned"/>
<input type="radio" class="radioBtns" name="returnForCredit" value="Y" id="returnForCredit2" onclick="checksearch()"/>
<fmt:message key="label.credit"/>
        </td>
        <td width="15%" class="optionTitleBoldRight" nowrap><fmt:message key="label.sortby"/>:</td>
        <td width="30%" class="optionTitleBoldLeft">
          <c:set var="selectedSortBy" value="${param.sortBy}"/>
          <c:if test="${selectedSortBy == null || selectedSortBy == ''}">
            <c:set var="selectedSortBy" value='RECEIPT_ID'/>
          </c:if>
          <html:select property="sortBy" styleId="sortBy" styleClass="selectBox" value="${selectedSortBy}">
            ...
            <html:option value="facilityId" key="label.facility"/>
      ...
      <tr>
        <td width="100%" class="optionTitleBoldLeft" colspan="4" nowarp="nowarp">
          ...
          <html:button property="buttonReprint" styleId="buttonReprint" styleClass="inputBtns" onmouseover="this.className='inputBtns inputBtnsOver'" onmouseout="this.className='inputBtns'" onclick="doReprint(); return false;">
