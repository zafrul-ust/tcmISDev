...
<xsl:variable name="invoiceDate" select="invoiceDate"/>
<w:t>Invoice for site <xsl:value-of select="facilityId"/>, <xsl:call-template name="format-date">
 <xsl:with-param name="date" select="$invoiceDate" />
...
<xsl:variable name="invoiceAmount" select="invoiceAmount"/>
<w:t>Total Invoice Amount for PO <xsl:value-of select="poNumber"/>: <xsl:value-of 
