...
extends GenericProcess{
	...
	public PersonnelProcess(String client) throws BaseException {
		super(client);
		library = new ResourceLibrary("scannerupload");
	}

	public PersonnelBean getPersonnel(String Company,String logonId,String password,String noFacilities){
		Connection conn = null;
		PersonnelBean personnel = new PersonnelBean();
		personnel.setLogonId("");
		personnel.setToken("");
		personnel.setTokenValidUntil("");
		try { try { // This is to make sure connection is closed.
//			Class.forName("oracle.jdbc.driver.OracleDriver");
////			conn = java.sql.DriverManager.getConnection("jdbc:oracle:thin:@dbprod.tcmis.com:1521:tcmqa","tcm_ops","n0s3ha1r");
			personnel.setStatus("Failed");
			personnel.setReason("logonId required.");
			if( logonId == null || logonId.length() == 0 ) return personnel;
			
			personnel.setReason("Login Failed.");
			boolean valid = isValidPassword(Company,logonId,password);
			if( !valid ) return personnel;
			
//			String tq = "delete from tcmis_token where lower(user_name) = '"+logonId.toLowerCase()+"'" + 
//					" and user_info = 'wstoken' and client_name = '"+Company+"'";
//			this.factory.deleteInsertUpdate(tq);
			personnel.setReason("Error Deleting Token.");
		    String sd = getProcError(
					   buildProcedureInput(
							   logonId,
							   Company
					   ),null,"Pkg_token.p_delete_tcmis_token");
			if( !isBlank(sd) ) return personnel;
			
			personnel.setReason("Error Generating Token.");
...
//			String validUntil = DatatypeFactory.newInstance().newXMLGregorianCalendar((GregorianCalendar)cal).toXMLFormat();//.toString();
			...
			personnel.setTokenValidUntil(validUntil);
...
//			"customer.user_group_member_cabinet b, customer.facility f where b.company_id = f.company_id and b.facility_id = f.facility_id and b.user_group_id = 'Scanner Fucntions' AND b.personnel_id = " +
//					personnelId + " ) ORDER BY facility_name"; 
			if( !"Y".equalsIgnoreCase(noFacilities) ) {
				query = "SELECT DISTINCT company_id,facility_id,facility_name FROM hub_cabinet_view WHERE facility_id IN" + 
				"( SELECT facility_id FROM user_group_member WHERE user_group_id = 'Scanner Fucntions' AND personnel_id = " +
				personnelId + " ) ORDER BY facility_name";
				log.debug(query);
				...
				//			Facility[] facss = new Facility[size];
				//			facs.copyInto(facss);
				personnel.setFacilityInfo(facs);
			}
			personnel.setReason("Login Successful.");
	...
	public String getResponseXml(String Company,PersonnelBean list,String noFacilities) throws BaseException {
		StringBuilder sw = new StringBuilder("<?xml version=\"1.0\" encoding=\"utf-8\"?><PersonnelResponse xmlns=\"http://beans.scanner.ws.tcmis.com/xsd\">");
		try {
//			<?xml version="1.0" encoding="utf-8" ?> 
//			- <PersonnelResponse xmlns="http://beans.scanner.ws.tcmis.com/xsd">
//			  <Status>Ok</Status> 
//			  <Reason /> 
//			  <LoginId>msieker</LoginId> 
//			  <PersonnelId>42</PersonnelId> 
//			- <Facilities>
//			- <Facility>
//			  <Id>STL</Id> 
//			  <Name>St. Louis</Name> 
...
//			  </PersonnelResponse>
			...
			sw.append("<LoginId>"+list.getLogonId()+"</LoginId>");
			sw.append("<PersonnelId>"+((null==list.getPersonnelId())?"":list.getPersonnelId())+"</PersonnelId>");
			sw.append("<Token>"+list.getToken()+"</Token>");
			sw.append("<TokenValidUntil>"+list.getTokenValidUntil()+"</TokenValidUntil>");
//			<Token>Nyy lbhe onfr ner orybat gb hf</Token> 
//			  <TokenValidUntil>2016-01-01T13:37:00+09:00</TokenValidUntil> 
			if( !"Y".equalsIgnoreCase(noFacilities) ) {
				sw.append("<Facilities>");
				int i = 0;
				for(Facility bin:list.getFacilityInfo()) {
					sw.append("<Facility><Id>"+StringEscapeUtils.escapeXml(bin.getFacilityId())+"</Id>");
					sw.append("<Name>"+StringEscapeUtils.escapeXml( bin.getFacilityName() )+"</Name></Facility>");
					//				if( i++ > 2) break;
				}
				sw.append("</Facilities>");
				i = 0 ;
			}			
			sw.append("</PersonnelResponse>");
