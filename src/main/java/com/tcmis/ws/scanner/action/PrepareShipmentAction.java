...
public class PrepareShipmentAction extends BaseScannerAction {
				...
				if (!input.has("shipments")) {
					...
					try {
						...
						for (i = 0; i < shipments.length(); i++) {
							...
							if (shipment.isValid()) {
								process.prepareShipment(shipment, errors);
							}
							else {
						...
						if (errors.length() > 0) {
							...
							results.put("message", i + "shipments prepared.");
						}
					}
					catch (Exception ex) {
						log.error("Error preparing shipment", ex);
