...
public class ReceiptIdSearchAction extends BaseScannerAction {
			...
			if (!isLoggedIn(request, true)) {
				...
				try {
					...
					if (isFilterValid(input)) {
						...
						ReceiptIdSearchProcess process = new ReceiptIdSearchProcess(getDbUser());

						try {
							results.put("receipts", process.getResults(input));
