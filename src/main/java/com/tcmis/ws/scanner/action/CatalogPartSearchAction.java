...
import com.tcmis.ws.scanner.process.CatalogPartSearchProcess;
import com.tcmis.ws.scanner.process.PartSearchProcess;

public class CatalogPartSearchAction extends BaseScannerAction {
			...
			if (!isLoggedIn(request, true)) {
				...
				try {
					...
					if (isFilterValid(input)) {
						CatalogPartSearchProcess process = new CatalogPartSearchProcess(getDbUser());

						try {
							results.put("catalogParts", process.getResults(input));
