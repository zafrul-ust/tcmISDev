...
public class ShipmentAddPickAction extends BaseScannerAction {
	...
	public ActionForward executeAction(ActionMapping mapping, HttpServletRequest request, HttpServletResponse response, String jsonString) throws BaseException {
		try {
				...
				if (!input.has("shipments")) {
					...
					try {
						...
						for (i = 0; i < shipments.length(); i++) {
							...
							if (shipment.isValid()) {
								process.addPicksToShipment(shipment, errors);
							}
							else {
								JSONObject error = new JSONObject();
								error.put("id", shipment.getId());
								error.put("errorMessage", "Requires tabletShipmentId && pickingUnits");
								errors.put(error);
							}
						}
						if (errors.length() > 0) {
							...
							results.put("message", i + " shipments had picks added.");
						}
					}
					catch (Exception ex) {
						log.error("Error adding picks to shipment", ex);
			...
			log.error("Base Error in PrepareShipmentAction", e);
