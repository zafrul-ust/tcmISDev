...
import com.tcmis.ws.scanner.process.CylinderLocationSearchProcess;
import com.tcmis.ws.scanner.process.CylinderSearchProcess;

public class CylinderSearchAction extends BaseScannerAction {
	...
	public ActionForward executeAction(ActionMapping mapping, HttpServletRequest request, HttpServletResponse response, String jsonString) throws BaseException {
		try {
			...
			if (!isLoggedIn(request, true)) {
				...
				try {
					...
					if (isFilterValid(input)) {
						CylinderSearchProcess process = new CylinderSearchProcess(getDbUser());

						try {
							results.put("cylinders", process.getCylinders(input));
			...
			log.error("Base Error in CylinderLocationSearchAction", e);
