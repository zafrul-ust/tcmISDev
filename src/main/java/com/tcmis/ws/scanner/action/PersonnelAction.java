...
//                            "Received order ", "ORDER:\n" + xmlString);
    ...
    if(xmlString != null && xmlString.length() > 0) {
        try {
        	PersonnelProcess process = new PersonnelProcess(this.getDbUser(request));
...
//        		System.out.println("facilityId:"+s);
        	...
        	String noFacilities = request.getParameter("NoFacilities");
            String result = process.processOrder(getPathCompany(request),xmlString,noFacilities);
...
//			String result = readFileAsString(outPath);
            ...
            e.printStackTrace();
            log.fatal("Error processing Personnel " + e.getMessage(), e);
