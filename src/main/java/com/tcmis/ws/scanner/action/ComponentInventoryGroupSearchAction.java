...
import com.tcmis.ws.scanner.process.ComponentInventoryGroupSearchProcess;
import com.tcmis.ws.scanner.process.PartSearchProcess;

public class ComponentInventoryGroupSearchAction extends BaseScannerAction {
			...
			if (!isLoggedIn(request, true)) {
				...
				try {
					...
					if (isFilterValid(input)) {
						...
						ComponentInventoryGroupSearchProcess process = new ComponentInventoryGroupSearchProcess(getDbUser());

						try {
							results.put("overrides", process.getComponentInventoryGroups(input));
