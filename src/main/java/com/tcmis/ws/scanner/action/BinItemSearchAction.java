...
public class BinItemSearchAction extends BaseScannerAction {
	...
	public ActionForward executeAction(ActionMapping mapping, HttpServletRequest request, HttpServletResponse response, String jsonString) throws BaseException {
		try {
				...
				try {
					...
					if (isFilterValid(input)) {
						if (!input.has("facilityId") || !input.has("companyId")) {
							...
							BinItemSearchProcess process = new BinItemSearchProcess(getDbUser());

							try {
								results.put("binItems", process.getBinItems(input));
			...
			log.error("Base Error in BinItemSearchAction", e);
