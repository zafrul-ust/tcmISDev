...
import com.tcmis.ws.scanner.process.CylinderManufacturerSearchProcess;

public class CylinderManufacturerSearchAction extends BaseScannerAction {
		...
		try {
			...
			if (!isLoggedIn(request, true)) {
				...
				try {
					...
					CylinderManufacturerSearchProcess process = new CylinderManufacturerSearchProcess(getDbUser());

					try {
						results.put("manufacturers", process.getCylinders(input));
			...
			log.error("Base Error in CylinderManufacturerSearchAction", e);
