...
import com.tcmis.ws.scanner.process.SupplierLocationSearchProcess;

public class SupplierLocationSearchAction extends BaseScannerAction {
	...
	public ActionForward executeAction(ActionMapping mapping, HttpServletRequest request, HttpServletResponse response, String jsonString) throws BaseException {
		try {
			...
			if (!isLoggedIn(request, true)) {
				...
				try {
					...
					if (!input.has("supplier")) {
						...
						results.put("errorMessage", "supplier is required");
						results.put("success", false);
					}
					else {

						SupplierLocationSearchProcess process = new SupplierLocationSearchProcess(getDbUser());

						try {
							results.put("locations", process.getLocations(input));
			...
			log.error("Base Error in SupplierSearchAction", e);
