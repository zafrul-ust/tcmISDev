...
import com.tcmis.ws.scanner.process.SupplierSearchProcess;

public class SupplierSearchAction extends BaseScannerAction {
		...
		try {
			...
			if (!isLoggedIn(request, true)) {
				...
				try {
					...
					SupplierSearchProcess process = new SupplierSearchProcess(getDbUser());

					try {
						results.put("suppliers", process.getSuppliers(input));
