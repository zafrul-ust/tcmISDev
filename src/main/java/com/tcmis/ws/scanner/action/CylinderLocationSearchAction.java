...
public class CylinderLocationSearchAction extends BaseScannerAction {
			...
			if (!isLoggedIn(request, true)) {
				...
				try {
					...
					if (isFilterValid(input)) {
						CylinderLocationSearchProcess process = new CylinderLocationSearchProcess(getDbUser());

						try {
							results.put("suppliers", process.getCylinderLocations(input));
