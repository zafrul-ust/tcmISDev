...
import com.tcmis.ws.scanner.process.CylinderRefurbCategorySearchProcess;

public class CylinderRefurbCategorySearchAction extends BaseScannerAction {
		...
		try {
			...
			if (!isLoggedIn(request, true)) {
				...
				try {
					...
					CylinderRefurbCategorySearchProcess process = new CylinderRefurbCategorySearchProcess(getDbUser());

					try {
						results.put("suppliers", process.getCylinderRefurbCategorys(input));
			...
			log.error("Base Error in CylinderRefurbCategorySearchAction", e);
