...
import com.tcmis.ws.scanner.process.ContainerSearchProcess;

public class ContainerSearchAction extends BaseScannerAction {
				...
				try {
					...
					if (!input.has("facilityId") || !input.has("companyId")) {
						...
						if (isFilterValid(input)) {
							...
							ContainerSearchProcess process = new ContainerSearchProcess(getDbUser());

							try {
								results.put("containers", process.getContainers(input)); 
