...
import com.tcmis.ws.scanner.process.ShipperSearchProcess;

public class ShipperSearchAction extends BaseScannerAction {
	...
	public ActionForward executeAction(ActionMapping mapping, HttpServletRequest request, HttpServletResponse response, String jsonString) throws BaseException {
		try {
			...
			if (!isLoggedIn(request, true)) {
				...
				try {
					...
					if (isFilterValid(input)) {
						ShipperSearchProcess process = new ShipperSearchProcess(getDbUser());

						try {
							results.put("shippers", process.getResults(input));
			...
			log.error("Base Error in ShipperSearchAction", e);
