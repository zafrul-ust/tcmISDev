...
public class ShipmentDepartureScanAction extends BaseScannerAction {
	...
	public ActionForward executeAction(ActionMapping mapping, HttpServletRequest request, HttpServletResponse response, String jsonString) throws BaseException {
		try {
					...
					try {
						...
						for (i = 0; i < shipments.length(); i++) {
							...
							if (shipment.hasTabletShipmentId()) {
								process.confirmShipment(shipment, errors);
							}
							else {
						...
						if (errors.length() > 0) {
							...
							results.put("message", "Shipment(s) confirmed.");
						}
					}
					catch (Exception ex) {
						log.error("Error confirming shipment", ex);
			...
			log.error("Base Error in ShipmentDepartureScanAction", e);
