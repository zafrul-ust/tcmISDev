...
import com.tcmis.ws.scanner.process.InventoryGroupSearchProcess;

public class InventoryGroupSearchAction extends BaseScannerAction {
	...
	public ActionForward executeAction(ActionMapping mapping, HttpServletRequest request, HttpServletResponse response, String jsonString) throws BaseException {
		try {
			...
			if (!isLoggedIn(request, true)) {
				...
				try {
					...
					if (isFilterValid(input)) {
						InventoryGroupSearchProcess process = new InventoryGroupSearchProcess(getDbUser());

						try {
							results.put("inventoryGroups", process.getResults(input));
			...
			log.error("Base Error in InventoryGroupSearchAction", e);
