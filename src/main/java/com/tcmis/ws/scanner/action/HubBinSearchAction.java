...
import com.tcmis.ws.scanner.process.HubBinSearchProcess;

public class HubBinSearchAction extends BaseScannerAction {
			...
			if (!isLoggedIn(request, true)) {
				...
				try {
					...
					if (!input.has("hub")) {
						...
						HubBinSearchProcess process = new HubBinSearchProcess(getDbUser());

						try {
							results.put("rooms", process.getResults(input));
