...
import com.tcmis.ws.scanner.process.ReceiptSearchProcess;

public class ReceiptSearchAction extends BaseScannerAction {
		...
		try {
			...
			if (!isLoggedIn(request, true)) {
				...
				try {
					...
					if (!input.has("facilityId") || !input.has("companyId")) {
						results.put("errorCode", RESPONSE_INVALID_INPUT);
						results.put("errorMessage", "companyId && facilityId are required");
						results.put("success", false);
					}
					else {
						if (isFilterValid(input)) {

							ReceiptSearchProcess process = new ReceiptSearchProcess(getDbUser());

							try {
								results.put("receipts", process.getReceipts(input));
			...
			log.error("Base Error in ReceiptSearchAction", e);
