...
import com.tcmis.ws.scanner.process.InboundShipmentSearchProcess;
import com.tcmis.ws.scanner.process.ReceiptIdSearchProcess;

public class InboundTransferSearchAction extends BaseScannerAction {
			...
			if (!isLoggedIn(request, true)) {
				...
				try {
					...
					if (!input.has("hub")) {
						...
						results.put("errorMessage", "hub is required");
						results.put("success", false);
					}
					else if (!isFilterValid(input)) {
						results.put("errorCode", RESPONSE_INVALID_INPUT);
						results.put("errorMessage", "filter is not in correct format: yyyy-MM-dd'T'HH:mm:ss");
						results.put("success", false);
					}
					else {
						InboundShipmentSearchProcess process = new InboundShipmentSearchProcess(getDbUser());

						try {
							results.put("transfers", process.getTransfers(input));
