...
import com.tcmis.ws.scanner.process.PickableUnitsSearchProcess;

public class PickableUnitsSearchAction extends BaseScannerAction {
	...
	public ActionForward executeAction(ActionMapping mapping, HttpServletRequest request, HttpServletResponse response, String jsonString) throws BaseException {
		try {
			...
			if (!isLoggedIn(request, true)) {
				...
				PickableUnitsSearchProcess process = new PickableUnitsSearchProcess(getDbUser());
				results = getPickableUnits(jsonString, process);
			}
			...
			log.error("Base Error in GetPickableUnitsAction", e);
			throw new BaseException(e);
		}
		return noForward;
	}
	
	public JSONObject getPickableUnits(String jsonString, PickableUnitsSearchProcess process) throws JSONException {
		JSONObject results = new JSONObject();
		try {
			JSONObject input = StringUtils.isNotBlank(jsonString) ? new JSONObject(jsonString) : new JSONObject();
			if (isFilterValid(input)) {
				try {
					JSONArray pickableUnitsArray = process.getPickableUnits(input);
					results.put("pickableUnits",pickableUnitsArray);
