...
public class VerifyFacilityActiveAction extends BaseScannerAction {
		...
		try {
			...
			if (!isLoggedIn(request, true)) {
				...
				try {
					...
					if (!input.has("companyId") || !input.has("facilityId")) {
						results.put("errorCode", RESPONSE_INVALID_INPUT);
						results.put("errorMessage", "companyId && facilityId are both required");
						results.put("success", false);
					}
					else {
						...
						try {
							results.put("active", process.isFacilityActiveForScanning(input));
			...
			log.error("Base Error in VerifyFacilityActiveAction", e);
