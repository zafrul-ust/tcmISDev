...
public class PoLineDetailAction extends BaseTabletAction {
	...
	public ActionForward executeAction(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (!isLoggedIn(request, true)) {
			...
			if (input.hasItemId() && input.hasInboundShipmentDetailId()) {
				...
				try {
					results.put("Lines", process.getInboundTransLineDetail(input, dateFormat));
				...
				results.put("Status", RESPONSE_ERROR);
				results.put("Message", "Both inboundShipmentDetailId & itemId are required");
			}
		}
