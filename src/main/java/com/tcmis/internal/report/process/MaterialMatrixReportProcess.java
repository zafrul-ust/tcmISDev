...
/******************************************************************************
 * Process to create Material Matrix excel report
 * @version 1.0
 *****************************************************************************/
public class MaterialMatrixReportProcess
    extends BaseExcelReportProcess {
  Log log = LogFactory.getLog(this.getClass());

  public MaterialMatrixReportProcess(String client) {
    super(client);
  }
   ...
   * Process to create Material Matrix excel report
       *****************************************************************************/
/*
  public HSSFWorkbook getReport(MaterialMatrixReportBean bean) throws
      BaseException,
      Exception {
    ...
    DataSet dataSet = factory.getDataSetForMaterialMatrix(bean);
    ...
    row.createCell( (short) 0).setCellValue("Material Matrix Adhoc Report");
    ...
    row.createCell( (short) 0).setCellValue("Parts:");
    row.getCell( (short) 0).setCellStyle(getRightBorderBoldFontCellStyle(wb));
    if (bean.getType().equalsIgnoreCase("All Approved")) {
      row.createCell( (short) 1).setCellValue(bean.getType());
    }
    else if (bean.getType().equalsIgnoreCase("Part Number")) {
      row.createCell( (short) 1).setCellValue(bean.getPartNo());
    }
    else if (bean.getType().equalsIgnoreCase("All Used")) {
      row.createCell( (short) 1).setCellValue(bean.getType());
