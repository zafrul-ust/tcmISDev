...
public class POReceiptDocumentAction
 ...
 public ActionForward executeAction(ActionMapping mapping,
	...
	if (!this.isLoggedIn(request,true)) {
	 request.setAttribute("requestedPage", "poreceiptdocuments");
	...
	if (form != null && receiptDocumentInputBean.getUpdateDocuments() != null &&
	 receiptDocumentInputBean.getUpdateDocuments().length() > 0) {
	 DynaBean dynaForm = (DynaBean) form;
	 List receiptDocumentViewBeans = (List) dynaForm.get("receiptDocumentViewBean");
	 ...
	 request.setAttribute("receiptDocumentViewBeanCollection", receiptDocumentProcess.getPOSearchResult(receiptDocumentInputBean));
	 return (mapping.findForward("showDocuments"));
	}
	else if (form != null && receiptDocumentInputBean.getSubmitSave() != null &&
	 receiptDocumentInputBean.getSubmitSave().length() > 0) {
	 ...
	 ReceiptDocumentProcess receiptDocumentProcess = new ReceiptDocumentProcess(this.getDbUser(request),getTcmISLocaleString(request));
	 addNewDocumentUrl = receiptDocumentProcess.addNewDocument(
		receiptDocumentInputBean);
	 if (addNewDocumentUrl != null && addNewDocumentUrl.length() > 0) {
		...
		vvDataProcess.getCompanyIdsForInventoryGroup(receiptDocumentInputBean.
		getInventoryGroup()));
