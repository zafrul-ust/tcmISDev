...
import com.tcmis.internal.invoice.factory.InvoiceViewBaeBeanFactory;

/***********************************************************
 * Process for creating BAE invoice.
 ***********************************************************/

public class BaeInvoiceProcess
    extends BaseProcess {
  Log log = LogFactory.getLog(this.getClass());

  public BaeInvoiceProcess(String client) {
    super(client);
  }
  ...
  public void getWordInvoice(InvoiceInputBean bean, Writer writer) throws
      ...
      InvoiceViewBaeBeanFactory factory = new InvoiceViewBaeBeanFactory(
          dbManager);
      Collection collection = factory.select(new BigDecimal("" +
      ...
      //unfortunately I have to loop thru and count invoices per invoice number
      Iterator iterator = collection.iterator();
      BigDecimal invoice = new BigDecimal("0");
      int count = 1;
      while(iterator.hasNext()) {
        InvoiceViewBaeBean b = (InvoiceViewBaeBean)iterator.next();
        if((invoice.intValue() - b.getInvoice().intValue()) != 0) {
          count = 1;
        }
        else {
          count++;
        ...
        b.setInvoiceCount(new BigDecimal("" + count));
      }
      ...
      File xslFile = new File(library.getString("word.baeinvoice.xsl"));
