 ...
 ***********************************************************/
public class GenericInvoiceProcess
    extends BaseProcess {
  Log log = LogFactory.getLog(this.getClass());

  public GenericInvoiceProcess(String client,Locale locale) {
    super(client,locale);
  }
  ...
  public void getWordInvoice(InvoiceInputBean bean, Writer writer) throws
      ...
      try {
    	  hardcode = library.getString(client+"_HDATA");
      } catch(Exception ex){}// do nothing.
      
      String companyId=library.getString(client+"_company");
      String invoiceGroup=library.getString(client+"_invoiceGroup");
      
      beanWriter.getBindingConfiguration().setMapIDs(false);
      beanWriter.enablePrettyPrint();
      DbManager dbManager = new DbManager(getClient());
      SearchCriteria criteria = new SearchCriteria("companyId", SearchCriterion.EQUALS, companyId );
      if( invoiceGroup != null && invoiceGroup.length() != 0 )
    	  criteria.addCriterion("invoiceGroup", SearchCriterion.EQUALS, invoiceGroup);
      ...
      Vector<InvoiceBean> c = (Vector<InvoiceBean>)factory.select(criteria);
      if (c == null || c.size() == 0) {
        throw new NoDataException("Query returned no data");
      }
      for(InvoiceBean ibean:c)
    	  ibean.setHardcode(hardcode);
      ...
      File xslFile = new File(library.getString(client+"_xsl"));
