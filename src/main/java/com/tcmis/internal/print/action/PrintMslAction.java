...
public class PrintMslAction
  ...
  while (iterator.hasNext()) {
   ...
   BeanHandler.copyAttributes(lazyBean, supplierPackingViewBean);
   if (supplierPackingViewBean.getOk() !=null && supplierPackingViewBean.getOk().length() > 0)
   {
    ...
    locationLabelPrinterCollection = zplDataProcess.getLocationLabelPrinter(personnelBean,labelInputBean);
	 ...
	 {        
     //labelUrl = labelProcess.buildPrintCaseMSL(personnelBean,labelInputBean,locationLabelPrinterCollection,suppPackViewBeanCollection);
	 }   
   else if (labelInputBean.getUserAction().equalsIgnoreCase("printPalletMSL"))
	 {
		 //labelUrl = labelProcess.buildPrintPalletMSL(personnelBean,labelInputBean,locationLabelPrinterCollection,suppPackViewBeanCollection);
	 }	 
	 else if (labelInputBean.getUserAction().equalsIgnoreCase("printPackingList"))
	 {
		 //labelUrl = labelProcess.buildPrintPackingList(personnelBean,labelInputBean,locationLabelPrinterCollection,suppPackViewBeanCollection);
	 }
