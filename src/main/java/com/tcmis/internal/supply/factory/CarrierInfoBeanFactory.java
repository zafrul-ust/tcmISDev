...
public class CarrierInfoBeanFactory extends BaseBeanFactory
{
	...
	public String ATTRIBUTE_MIN_GROSS_SHIPMENT_WEIGHT = "MIN_GROSS_SHIPMENT_WEIGHT";
	public String ATTRIBUTE_MAX_GROSS_SHIPMENT_WEIGHT = "MAX_GROSS_SHIPMENT_WEIGHT";
	public String ATTRIBUTE_MAX_GROSS_INDIVIDUAL_WEIGHT = "MAX_GROSS_INDIVIDUAL_WEIGHT";
 ...
 values (
	...
	public int update(CarrierInfoBean carrierInfoBean, Connection conn)
		...
		String query  = "update " + TABLE + " set " +
			...
			"where " + ATTRIBUTE_CARRIER_CODE + "=" +
				StringHandler.nullIfZero(carrierInfoBean.getCarrierCode());
	...
	public Collection select(String searchArgument, String inventoryGroup)	throws BaseException
	{
		//		log.debug("using new version of CarrierInfoBeanFactory...");
		Connection connection = null;
		Collection c = null;
		try
		{
			connection = this.getDbManager().getConnection();
			c = select(searchArgument, inventoryGroup, connection);
		}
		finally
		{
			this.getDbManager().returnConnection(connection);
	...
	public Collection select(String searchArgument, String inventoryGroup, Connection conn)	throws BaseException
	{
		Collection carrierInfoBeanColl = new Vector();

		StringBuilder queryBuffer = new StringBuilder("select * from carrier_info where lower(CARRIER_CODE||CARRIER_NAME) like lower('%");
		queryBuffer.append(searchArgument);
		queryBuffer.append("%') and (inventory_group='");
		queryBuffer.append(inventoryGroup);
		queryBuffer.append("' or inventory_group='ALL')  and status='ACTIVE' order by company_id, ship_to_loc_id, carrier_name, carrier_code");
		...
		while (dataIter.hasNext())
		{
			...
			CarrierInfoBean carrierInfoBean = new CarrierInfoBean();
			load(dataSetRow, carrierInfoBean);
			//log.debug("carrierInfoBean.toString() = [" + carrierInfoBean.toString() + "]; ");
			carrierInfoBeanColl.add(carrierInfoBean);
		}
		return carrierInfoBeanColl;
	}
