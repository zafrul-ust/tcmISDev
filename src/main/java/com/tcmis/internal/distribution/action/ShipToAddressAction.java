   ...
   * @version 1.0
   ******************************************************************************/
  public class ShipToAddressAction
...
//  login
   	...
   	if (!this.isLoggedIn(request)) {
			request.setAttribute("requestedPage", "shiptoaddress");
...
*/
    ...
    CustLookUpProcess process = new CustLookUpProcess(this.getDbUser(request),getTcmISLocaleString(request));
    String action = request.getParameter("uAction");
    if (action != null && "search".equals(action)) {
       request.setAttribute("customerAddressColl",process.getSearchResult(inputBean, personnelBean));
    }
