 ...
 * @version 1.0
     ******************************************************************************/
public class CustomerRequestUpdateAction
  ...
  public ActionForward executeAction(ActionMapping mapping, ActionForm form,
    ...
    if (!this.isLoggedIn(request)) {
      request.setAttribute("requestedPage", "customerrequestupdate");
...
*/
    ...
    request.setAttribute("vvSapIndustryCollection", process.getSapIndustryKeyDropDown() );
    ...
    request.setAttribute("vvLocale", process.getVvLocaleDropDown());
...
//			}
		...
		beancoll.add( BeanHandler.getBeans((DynaBean)form,"CustomerTaxAddRequestBean",new CustomerTaxExemptAddReqBean(),this.getTcmISLocale()));
// conflict grid column name problem.		
		Vector<CustEntPriceGpAddRequestBean> cv = (Vector<CustEntPriceGpAddRequestBean>)BeanHandler.getBeans((DynaBean)form,"billColl",new CustEntPriceGpAddRequestBean(),this.getTcmISLocale());
		for(CustEntPriceGpAddRequestBean cvb:cv ) {
			cvb.setOpsEntityId(cvb.getBillopsEntityId());
			cvb.setPriceGroupId(cvb.getBillpriceGroupId());
			cvb.setCustomerRequestId(sbean.getCustomerRequestId());
			cvb.setBillToCompanyId(sbean.getCompanyId());
			cvb.setCustomerId(sbean.getCustomerId());
		}
		beancoll.add( cv );
...
//    request.setAttribute("customerAddRequestViewBean", sbean);
    ...
    request.setAttribute("taxColl",process.getCustomerTax(sbean.getCustomerRequestId()));
    request.setAttribute("billColl",process.getCustomerEntityPG(sbean.getCustomerRequestId()));
    request.setAttribute("labelColl", process.getSearchResultObjects( "select distinct LABEL_FORMAT from INVENTORY_GROUP_LABEL_FORMAT x where LABEL_TYPE = 'exitLabel'" ) );
    return mapping.findForward(forward);
  }
}