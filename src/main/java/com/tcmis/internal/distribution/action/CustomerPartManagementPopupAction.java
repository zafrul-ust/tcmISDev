...
public class CustomerPartManagementPopupAction extends TcmISBaseAction
{
	...
	public ActionForward executeAction(ActionMapping mapping,
		...
		if (!this.isLoggedIn(request)) {
			request.setAttribute("requestedPage", "customerpartmanagementpopup");
	   ...
	   CustomerPartManagementProcess process = new CustomerPartManagementProcess(this.getDbUser(request),getTcmISLocaleString(request));
		...
		if (action.equals("update") ) {
			...
			checkToken(request);
			Collection beans = BeanHandler.getBeans((DynaBean) form, "customerPartManagementPopupDiv", new CustomerPartManagementInputBean(), this.getTcmISLocale(request));
			process.updateData(inputBean,beans);
			//research data
