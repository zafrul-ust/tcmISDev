 ...
 * @version 1.0
     ******************************************************************************/
public class NewShipToAddressAction
  ...
  public ActionForward executeAction(ActionMapping mapping, ActionForm form,
    ...
    if (!this.isLoggedIn(request)) {
      request.setAttribute("requestedPage", "newshiptoaddress");
...
//    }
    ...
    NewShipToAddressProcess process = new NewShipToAddressProcess(this);
  
    NewShipToAddressInputBean inputBean = new NewShipToAddressInputBean();
    ...
    if ("new".equals(uAction) ) {
      /*if (!personnelBean.getPermissionBean().hasFacilityPermission("newRemitTo",null,null))
      {
    	...
    	Object[] results = process.callCreateShipTo(inputBean,personnelBean);
    	String err = results[0].toString();
    	if(err.trim().length() > 0) {
    		request.setAttribute("tcmISError", err);
    	}
    	else {
	        String shiptoId = results[1].toString();
	    	CustomerEntityPGProcess shprocess = new CustomerEntityPGProcess(this);
			...
			for (OpsEntityFacilityViewBean bean : beans) {
				bean.setFacilityId(shiptoId);
	// I don't know where to get the companyId			
				bean.setCompanyId(inputBean.getBillToCompanyId());
				if ("New".equals(bean.getChanged()))
					err += shprocess.addShipToPG(bean,personnelId);
				else if ("Remove".equals(bean.getChanged()))
					err += shprocess.removeShipToPG(bean);
				else
					err += shprocess.updateShipToPG(bean,personnelId);
			}
	    	if (err.trim().length() > 0)
	    		request.setAttribute("tcmISError", err);
	    	else
	...
	// I don't know where to get the companyId			
	      	...
	      	request.setAttribute("shipToLocationId",results[1] );
			
			inputBean.setShipToLocationId((BigDecimal) results[1]);
	      	 Collection coll = process.getShipToAddress(inputBean);
			    if( (coll.size() != 0) && (err.trim().length()==0) )
				{	
				 request.setAttribute("newShipToBean",coll.toArray()[0]);
				}
    	}
    }
    else {
