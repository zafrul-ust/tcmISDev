...
public class CustomerOrderTrackingProcess extends GenericProcess {
	...
	throws BaseException {
		...
		String query="select * from table (pkg_open_customer.FX_CUSTOMER_MR_LINE_SEARCH(" +
		where  +		
	...
	public ExcelHandler getOpenCustomerExcelReport(CustomerOrderTrackingInputBean inputBean,
		...
		String[] headerkeys = {
				"label.operatingentity", "label.purchaseorder", "label.haasreference", "label.requestor", "label.customerpartnumber",
		...
		for (SalesOrderViewBean salesOrderBean : salesOrderCollection) {
			...
			pw.addCell(salesOrderBean.getCustomerPartNo());
