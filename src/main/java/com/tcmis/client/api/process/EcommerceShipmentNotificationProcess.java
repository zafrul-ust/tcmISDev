...
public class EcommerceShipmentNotificationProcess extends BaseProcess {
    ...
    public JSONArray getShipmentNotificationData(JSONObject input) throws Exception {
        ...
        try {
            ...
            while (iter.hasNext()) {
                ...
                shipNoticePortionJSON.put("OrderRefernce",orderReferenceJSON);
                //
  ...
  private Vector getShipmentData (JSONObject input) throws Exception {
      ...
      //todo need to talk with Sonny to finalize the frequency
      String limitTimeFrame = ".2";
      String prNumberString = "";
      try {
          limitTimeFrame = input.getString("minuteSince");
          prNumberString = input.getString("prNumbers");
      } catch (Exception ee) {/*minuteSince is only use for testing*/}
      ...
      query.append(" where ship_confirm_date > sysdate -").append(limitTimeFrame);
  ...
  private void getShipNoticeItem(JSONArray shipNoticeItemArray, EcommerceShipmentNotificationBean ecommerceShipmentNotificationBean) throws Exception {
      ...
      //
      shipNoticeItemDetailJSON.put("UnitOfMeassure","EA");
  ...
  private void getPostalJSON(EcommerceShipmentNotificationBean ecommerceShipmentNotificationBean, JSONObject postalAddressJSON, String toHeaderValue) throws Exception {
      if (!StringHandler.isBlankString(toHeaderValue)) {
          String[] deliverTo = {StringHandler.emptyIfNull(ecommerceShipmentNotificationBean.getEndUser()), toHeaderValue};
          postalAddressJSON.put("DeliverTo", deliverTo);
      }else
          postalAddressJSON.put("DeliverTo", "");
      //
      ...
      postalAddressJSON.put("Street",streetAddress.toString());
