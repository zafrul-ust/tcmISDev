...
import com.tcmis.client.report.process.FormattedMsdsRevisionProcess;
 ...
 */
public class FormattedMsdsRevisionAction extends TcmISBaseAction {

  public ActionForward executeAction(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws BaseException, Exception {

    if (!this.isLoggedIn(request)) {
      request.setAttribute("requestedPage", "formattedmsdsrevisionreport");
    ...
    if(form != null) {
        ...
        if ("submit".equalsIgnoreCase(inputBean.getSubmitValue())) {
            if (log.isDebugEnabled()) {
              log.debug("Running report:"+inputBean);
            }
            this.setExcelHeader(response);
            FormattedMsdsRevisionProcess process = new FormattedMsdsRevisionProcess(this.getDbUser(request));
