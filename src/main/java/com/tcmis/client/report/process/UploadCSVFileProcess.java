...
/******************************************************************************
 * Process to upload CSV file to our server.
 * @version 1.0
 *****************************************************************************/
public class UploadCSVFileProcess extends BaseProcess {
	Log log = LogFactory.getLog(this.getClass());
	ResourceLibrary resource = new ResourceLibrary("tcmISWebResource");
	ResourceLibrary library = new ResourceLibrary("com.tcmis.common.resources.CommonResources", this.getLocaleObject());

    public UploadCSVFileProcess(String client) {
		super(client);
	}

    public String getTradeName (GtImportConsumptionStageBean bean, String facilityId, String uploadSeqId) throws BaseException {
        String result = "";
        try {
            ...
            StringBuilder query = new StringBuilder("select m.trade_name from import_consumption ic, material m");
    ...
    public Object[] saveFile(ScannerUploadInputBean bean,String companyId,String facilityId,String application,String personnelId) throws BaseException {
    	...
    	if (bean.getTheFile() != null ) {
    		...
    		if(log.isDebugEnabled()) {
				log.debug("Save CVS file to database.");
    		}
    		...
    		try { 
                ...
                //do this to make sure we are not picking up any unneeded data
                sql.update(conn,"delete from GT_IMPORT_CONSUMPTION_STAGE");
                while( ( lineData = br.readLine()) != null) {
					lineNumber++;
					if( lineNumber <=1 ) continue;
					String[] cellValue = lineData.split(",");
					StringBuilder query = new StringBuilder("insert into GT_IMPORT_CONSUMPTION_STAGE (USAGE_DATE, WORK_AREA_NAME, MATERIAL_ID ,CUSTOMER_MSDS_OR_MIXTURE_NO, MSDS_UNIT_OF_MEASURE, MSDS_UNIT_AMOUNT, PART_NO, PART_AMOUNT, PURCHASE_SOURCE,APPLICATION,COMPANY_ID,FACILITY_ID,INSERTED_BY,INSERT_DATE,ENTRY_TYPE,DATA_LOAD_ROW_ID) values (");
                    ...
                    query.append(StringHandler.emptyIfNull(personnelId)).append(",sysdate,'I',"+lineNumber+")");
					sql.update(conn, query.toString());
				}

				 Vector out = new Vector();
				 out.add( new Integer(java.sql.Types.INTEGER) );
				 Vector seqIdV = (Vector) sql.doProcedure(conn, "PKG_IMPORT_CONSUMPTION.P_LOAD_IMPORT_CONSUMPTION",new Vector(),out);
				 if( seqIdV != null && seqIdV.size() == 1 && seqIdV.get(0) != null) {
					 seqId = seqIdV.get(0)+"";
					 errMsg += "Upload Successful!!<br/>";
					 log.debug("upload seq id:"+seqIdV.get(0).toString());
				 } else {
					 String q = "select DATA_LOAD_ROW_ID, ERROR_STATEMENT from GT_IMPORT_CONSUMPTION_ERROR";
    ...
    public Object[] saveChem(GtImportConsumptionStageBean bean,String companyId,String facilityId,String personnelId) throws BaseException {
    	String errMsg = "";
    	String seqId = ""; 
        DbManager dbManager = new DbManager(getClient(),getLocale());
        Connection conn = dbManager.getConnection();
        ...
        try {
            ...
            sql.update(conn,"delete from GT_IMPORT_CONSUMPTION_STAGE");
            StringBuilder query = new StringBuilder("insert into GT_IMPORT_CONSUMPTION_STAGE (USAGE_DATE, WORK_AREA_NAME, MATERIAL_ID ,CUSTOMER_MSDS_OR_MIXTURE_NO, MSDS_UNIT_OF_MEASURE, MSDS_UNIT_AMOUNT, PART_NO, PART_AMOUNT, PURCHASE_SOURCE,APPLICATION,COMPANY_ID,FACILITY_ID,INSERTED_BY,INSERT_DATE,ENTRY_TYPE,DATA_LOAD_ROW_ID) values (");
            query.append( GenericProcess.getSqlString(bean.getUsageDate()) +",'");
            ...
            if( seqIdV != null && seqIdV.size() == 1 && seqIdV.get(0) != null) {
                 ...
                 DataSet result =  sql.select(conn,"select DATA_LOAD_ROW_ID, ERROR_STATEMENT from GT_IMPORT_CONSUMPTION_ERROR");
