 ...
 * @version 1.0
 *****************************************************************************/
public class FormattedUsageProcess extends BaseProcess {
  Log log = LogFactory.getLog(this.getClass());

  public FormattedUsageProcess(String client) {
    super(client);
  }
...
*/
    // search type
    boolean chemList = "list".equalsIgnoreCase(inputBean.getReportType());
    boolean all = "all".equalsIgnoreCase(inputBean.getReportType());
    boolean kw = "keyword".equalsIgnoreCase(inputBean.getReportType());
    boolean singleChem = "singleChemical".equalsIgnoreCase(inputBean.getReportType());
...
*/
    ...
    if(chemList){
      where = where + "and list_id = '"+inputBean.getListOption()+"' ";
    }
    if(singleChem){
      where = where + "and cas_number = '"+inputBean.getCasNumber()+"' ";
    }
...
*/
    ...
    //com.tcmis.common.admin.process.MailProcess.sendEmail("rachord@haastcm.com","", "adhoc", "matrix report query", query);
    ...
    //it to the stream
      ...
      writer.write("<th>"+library.getString("label.casnumbersaragroup")+"</th>");
      writer.write("<th>"+library.getString("label.constituent")+"</th>");
      writer.write("<th>"+library.getString("label.partnumber")+"</th>");
      writer.write("<th>"+library.getString("label.tradename")+"</th>");
      writer.write("<th>"+library.getString("label.deliverypoint")+"</th>");
      ...
      writer.write("<th>"+library.getString("label.percentweightconstituent")+"</th>");
      ...
      for (int j = 1; j <= reportDataSet.getRowCount(); j++) {
        ...
        writer.write("<td>" + StringHandler.emptyIfNull(row.getString("CAS_NUMBER")) + "</td>");
        ...
        writer.write("<td>" + StringHandler.emptyIfNull(row.getString("TRADE_NAME")) + "</td>");
        writer.write("<td>" + StringHandler.emptyIfNull(row.getString("DELIVERY_POINT_DISPLAY")) + "</td>");
        ...
        writer.write("<td>" + StringHandler.emptyIfNull(row.getString("PERCENT")) + "</td>");
...
*/
      ...
      writer.write(library.getString("label.chemical") + " ");

      writer.write("</td>");
      writer.write("<td >");
      if("list".equalsIgnoreCase(bean.getReportType())){
          writer.write(bean.getChemicalListName());
      }
      else if("singleChemical".equalsIgnoreCase(bean.getReportType())){
          writer.write(bean.getCasNumber());
      }
      else {
          writer.write(library.getString("label.all"));
      }
      writer.write("</td>");
      writer.write("</tr>");
      writer.write("<tr>");
