package com.tcmis.client.common.process;

import java.io.File;
import java.io.FileInputStream;
import java.math.BigDecimal;
import java.sql.Connection;
import java.util.ArrayList;
import java.util.Date;
import java.util.Vector;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

import com.tcmis.common.admin.beans.PersonnelBean;
import com.tcmis.common.admin.process.MailProcess;
import com.tcmis.common.db.DbManager;
import com.tcmis.common.exceptions.BaseException;
import com.tcmis.common.framework.GenericProcess;
import com.tcmis.common.framework.GenericSqlFactory;
import com.tcmis.common.util.DateHandler;
import com.tcmis.common.util.ExcelHandler;
import com.tcmis.common.util.FileHandler;
import com.tcmis.common.util.ResourceLibrary;
import com.tcmis.common.util.SqlHandler;
import com.tcmis.common.util.StringHandler;
import com.tcmis.supplier.shipping.beans.ScannerUploadInputBean;


public class UploadPartLevelProcess extends GenericProcess {
	
	Log log = LogFactory.getLog(this.getClass());
	ResourceLibrary library = new ResourceLibrary("com.tcmis.common.resources.CommonResources", this.getLocaleObject());
	ResourceLibrary directoryName = new ResourceLibrary("scannerupload");
	DbManager dbManager;
    Connection connection = null;
    GenericSqlFactory genericSqlFactory;
    
    public UploadPartLevelProcess(String client) {
		super(client);
	}
   
	public static boolean isValidInt(String s) {
		try {
			int number = (new BigDecimal(s)).intValue();	
		}
		catch (Exception e) {
			return false;
		}
		return true;

	} //end testValidInt
    
	public static boolean isNullOrEmpty(String s) {
		if (s == null || s.trim().equals(""))
			return true;
		else 
			return false;

	} //end isNull
	
    public ArrayList<String> saveExcelFile(ScannerUploadInputBean inputBean, PersonnelBean pbean) throws BaseException 
    {
		ArrayList<String> returnMessage =  new ArrayList<String>();
		
		int row = 0;
		int count = 0;
		int lastRow = 0;
		if (inputBean.getTheFile() == null) {
			returnMessage.add("EXCEPTION");
			returnMessage.add("<br/>File is Empty");
			return returnMessage;
		}
		if (log.isDebugEnabled()) {
			log.debug(inputBean.getTheFile().getName());
		}
		
		try {
			dbManager = new DbManager(this.getClient(), this.getLocale());						
		    connection = dbManager.getConnection();	                
	        genericSqlFactory = new GenericSqlFactory(dbManager);
	        
			//reading the file and putting the values in a bean
			String dirname = directoryName.getString("upload.backupdir");
			File dir = new File(dirname);
			File f = File.createTempFile("tmpUploadPartLevel_", ".xls", dir);
			FileHandler.copy(inputBean.getTheFile(), f);					
			Vector<Vector<String>> excelData = ExcelHandler.read(new FileInputStream(f));
			
			//create the insert query for the temporary table		
			StringBuffer query = new StringBuffer("INSERT ALL ");;
			ArrayList<String> queries = new ArrayList<String>();
			
			//Capture the row errors and add them to file error
			StringBuilder lineError;
			StringBuilder fileError = new StringBuilder();

			for( Vector<String> v : excelData ) 
			{
				lineError = new StringBuilder();
				row++;				
				if( v == null  )  continue;				
				if( row == 1 ) {
					if (!validHeader(v)) {						 
						returnMessage.add("EXCEPTION");
						returnMessage.add(library.getString("error.upload.invalidHeader"));
						return returnMessage;
					}
					continue;
				}
								
				boolean dataIncorrect = false;
				Date levelHoldEndDate = null;				
				String stockingLevel = null;
				String reorderPoint = null;
				String binId = null;
				String partGroupNo = null;
				lastRow = excelData.size();
				
				//if there are less than 10 columns in a row return with exception
				if (v.size() < 10 ) {
					returnMessage.add("EXCEPTION");
					returnMessage.add(library.getString("error.upload.datamissing"));
					return returnMessage;
				}
				//check for null or empty string. all the fields are mandatory except the levelHoldEndDate
				if (isNullOrEmpty((String)v.get(0))) {
					if(lineError.length() > 0)
						lineError.append(", ");
					lineError.append("COMPANY_ID");					
				}
				
				if (isNullOrEmpty((String)v.get(1))) {
					if(lineError.length() > 0)
						lineError.append(", ");
					lineError.append("CATALOG_COMPANY_ID");					
				}
				
				if (isNullOrEmpty((String)v.get(2))) {
					if(lineError.length() > 0)
						lineError.append(", ");
					lineError.append("CATALOG_ID");					
				}
				
				if (isNullOrEmpty((String)v.get(3))) {
					if(lineError.length() > 0)
						lineError.append(", ");
					lineError.append("CAT_PART_NO");					
				} 
				
				if (!isValidInt((String)v.get(4))) {
					if(lineError.length() > 0)
						lineError.append(", ");
					lineError.append("PART_GROUP_NO");					
				} else
					partGroupNo = (new BigDecimal(v.get(4))).intValue() + "";
				
				if (isNullOrEmpty((String)v.get(5))) {
					if(lineError.length() > 0)
						lineError.append(", ");
					lineError.append("FACILITY_ID");					
				}
				
				if (isNullOrEmpty((String)v.get(6))) {
					if(lineError.length() > 0)
						lineError.append(", ");
					lineError.append("APPLICATION");					
				}
				
				if (!isValidInt((String)v.get(7))) {
					if(lineError.length() > 0)
						lineError.append(", ");
					lineError.append("BIN_ID");					
				} else
					binId = (new BigDecimal(v.get(7))).intValue() + "";
				
				if (!isValidInt((String)v.get(8))) {
					if(lineError.length() > 0)
						lineError.append(", ");
					lineError.append("REORDER_POINT");
					dataIncorrect = true;
				} else {
                    int tmpVal = (new BigDecimal(v.get(8))).intValue();
                    if (tmpVal < 0) {
                        if(lineError.length() > 0)
						    lineError.append(", ");
					    lineError.append("REORDER_POINT");
					    dataIncorrect = true;
                    }else
                        reorderPoint = tmpVal + "";
                }

                if (!isValidInt((String)v.get(9))) {
					if(lineError.length() > 0)
						lineError.append(", ");
					lineError.append("STOCKING_LEVEL");
					dataIncorrect = true;
				} else {
                    int tmpVal = (new BigDecimal(v.get(9))).intValue();
                    if (tmpVal < 0) {
                        if(lineError.length() > 0)
						    lineError.append(", ");
					    lineError.append("STOCKING_LEVEL");
					    dataIncorrect = true;
                    }else
                        stockingLevel = tmpVal + "";
                }
				//check if reorder point <= stocking level
				if (!dataIncorrect && !(Integer.parseInt(reorderPoint) <= Integer.parseInt(stockingLevel))) {
					if(lineError.length() > 0)
						lineError.append(", ");
					lineError.append("REORDER_POINT should be <= STOCKING_LEVEL");
				} 
				
				if (v.size() == 11) {
					if ((String)v.get(10) != null && !((String)v.get(10)).trim().equals("")) {
						if (StringHandler.isValidDouble((String)v.get(10))) {
							levelHoldEndDate = ExcelHandler.getDate((int)Double.parseDouble(v.get(10)));
						} else {
							if(lineError.length() > 0)
								lineError.append(", ");
							lineError.append("LEVEL_HOLD_END_DATE");
						}
					}
				}
								
				// only build the query if there are no errors
				if(lineError.length() > 0)
					fileError.append("Row " + row + ": Invalid data found: " + lineError.toString() + "\n");
				else if(lineError.length() < 1) {
					count++;	
										
					query.append("INTO GT_CABINET_PART_LEVEL_STAGE(COMPANY_ID,CATALOG_COMPANY_ID,CATALOG_ID,CAT_PART_NO," +
							"PART_GROUP_NO,FACILITY_ID,APPLICATION,BIN_ID,REORDER_POINT,STOCKING_LEVEL,LEVEL_HOLD_END_DATE) ");
					query.append("VALUES (" + SqlHandler.delimitString(v.get(0)) + ","  + SqlHandler.delimitString(v.get(1)) + ","  + SqlHandler.delimitString(v.get(2)) 
							+ ","  + SqlHandler.delimitString(v.get(3)) + ","  + partGroupNo + ","  + SqlHandler.delimitString(v.get(5)) + ","  
							+ SqlHandler.delimitString(v.get(6)) + ","  + binId + ","  + reorderPoint + "," + stockingLevel + "," + DateHandler.getOracleToDateFunction(levelHoldEndDate) + ")\n");
					
					if( row % 100 == 0 || (row == excelData.size())) {
						query.append(" SELECT * FROM DUAL ");
						queries.add(query.toString());
						query = new StringBuffer("INSERT ALL ");
					}
				}
			}
		
			if (fileError.length() > 0) {				
				returnMessage.add("MISMATCH");			
				returnMessage.add(library.getString("error.upload.mismatch"));
				
				//send email to the logged in user for any mismatch found
				StringBuffer messageBody = new StringBuffer();
				messageBody.append("Following errors were encountered during the upload\n");
				messageBody.append(fileError.toString() + "\n");
				MailProcess.sendEmail(pbean.getEmail(), null, MailProcess.DEFAULT_FROM_EMAIL, "UploadPartLevel failed", messageBody.toString());
			} else if( count > 0 ) {
				connection.setAutoCommit(false); 
				int rowsInserted = 0;
				//insert only if there are no errors and there is at least one row to be inserted
				for (int i=0; i < queries.size(); i++) {					
					rowsInserted += genericSqlFactory.deleteInsertUpdate(queries.get(i).toString(), connection);
				}
				String outMessage = null;
				Vector inArgs = new Vector();
				Vector outArgs = new Vector();
				inArgs.add(new Integer (pbean.getPersonnelId()));
				if (pbean.isOperatingCompanyEmployee()) 
					inArgs.add("Y");
				else
					inArgs.add("N");
	            outArgs.add(new Integer(java.sql.Types.VARCHAR));
	            //invoke procedure to compare the excel data
				Vector procErrorMessage = (Vector)genericSqlFactory.doProcedure(connection, "PKG_WORK_AREA_MANAGEMENT.P_CABINET_PART_LEVEL_LOAD", inArgs, outArgs);
				connection.commit();
				connection.setAutoCommit(true);
				//return procedure message back to the user 
				if (procErrorMessage != null && procErrorMessage.size() > 0) {					
					outMessage = (String) procErrorMessage.get(0);
					log.debug("Procedure output - " + outMessage);
					returnMessage.add(outMessage);
					if (outMessage.equalsIgnoreCase("OK")){
						returnMessage.add(library.getString("upload.successful"));
					} else if (outMessage.equalsIgnoreCase("MISMATCH")){
						returnMessage.add(library.getString("error.upload.mismatch"));						
					} else if (outMessage.equalsIgnoreCase("EXCEPTION")){
						returnMessage.add(library.getString("error.upload.generic"));						
					} else if (outMessage.equalsIgnoreCase("INVALID_PERMISSION")){
						returnMessage.add(library.getString("error.upload.nopermission"));						
					}
	            }
			}
		}
		catch (Exception ex) {
			returnMessage.add("EXCEPTION");
			returnMessage.add(library.getString("error.upload.generic"));
			log.error("Exception thrown while processing the file", ex);
		} finally {			
            dbManager.returnConnection(connection);
			dbManager = null;
			connection = null;
			genericSqlFactory = null;
		}
		return returnMessage;
	}

    //Validate the header
	private boolean validHeader(Vector<String> v) {
		boolean blnValidHeader = true;

		try {
			if(!((String)v.get(0)).equalsIgnoreCase("COMPANY_ID")) {
				blnValidHeader = false;
				return blnValidHeader;
			}
			if(!((String)v.get(1)).equalsIgnoreCase("CATALOG_COMPANY_ID")) {
				blnValidHeader = false;
				return blnValidHeader;
			}
			if(!((String)v.get(2)).equalsIgnoreCase("CATALOG_ID")) {
				blnValidHeader = false;
				return blnValidHeader;
			}
			if(!((String)v.get(3)).equalsIgnoreCase("CAT_PART_NO")) {
				blnValidHeader = false;
				return blnValidHeader;
			}
			if(!((String)v.get(4)).equalsIgnoreCase("PART_GROUP_NO")) {
				blnValidHeader = false;
				return blnValidHeader;
			}
			if(!((String)v.get(5)).equalsIgnoreCase("FACILITY_ID")) {
				blnValidHeader = false;
				return blnValidHeader;
			}
			if(!((String)v.get(6)).equalsIgnoreCase("APPLICATION")) {
				blnValidHeader = false;
				return blnValidHeader;
			}
			if(!((String)v.get(7)).equalsIgnoreCase("BIN_ID")) {
				blnValidHeader = false;
				return blnValidHeader;
			}
			if(!((String)v.get(8)).equalsIgnoreCase("REORDER_POINT")) {
				blnValidHeader = false;
				return blnValidHeader;
			}
			if(!((String)v.get(9)).equalsIgnoreCase("STOCKING_LEVEL")) {
				blnValidHeader = false;
				return blnValidHeader;
			}
			if(!((String)v.get(10)).equalsIgnoreCase("LEVEL_HOLD_END_DATE")) {
				blnValidHeader = false;
				return blnValidHeader;
			}
		} catch (Exception e) {
			blnValidHeader = false;
		}
		
		return blnValidHeader;
	}    
   
}
