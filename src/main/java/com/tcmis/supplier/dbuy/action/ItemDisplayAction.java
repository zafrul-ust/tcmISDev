...
public class ItemDisplayAction extends TcmISBaseAction {
	...
	public ActionForward executeAction(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		Collection contractItems = new Vector();
		...
		try {
			...
			if (fromForm!=null) contractItems = contractProcess.getDbuyItems(itemId,supplierId,inventoryGroup,status);
		} catch (BaseException be) {
			log.error("Base Excpetion in getSuppliers/getInvGrp/getItems");
			forward = "error";
		} finally {
		...
		// set new beans
		session.setAttribute("DbuyItemBeans",contractItems);
