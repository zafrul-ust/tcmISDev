...
public class PrintLabelsAction
  ...
  while (iterator.hasNext()) {
   ...
   BeanHandler.copyAttributes(lazyBean, supplierPackingSummaryViewBean,getTcmISLocale(request));
   if (supplierPackingSummaryViewBean.getOk() !=null && supplierPackingSummaryViewBean.getOk().length() >0)
   {
       //log.info(""+supplierPackingSummaryViewBean.getPrNumber()+"   "+supplierPackingSummaryViewBean.getLineItem()+"");
       suppPackSummaryViewBeanCollection.add(supplierPackingSummaryViewBean);
		...
		supplierPackingSummaryViewBean.setPicklistId(new BigDecimal("183664"));
  	 suppPackSummaryViewBeanCollection.add(supplierPackingSummaryViewBean);*/
	...
	if (suppPackSummaryViewBeanCollection.size() > 0) {
	 ...
	 String labelUrl = labelProcess.buildUnitBoxLabels(personnelBean,labelInputBean,locationLabelPrinterCollection,suppPackSummaryViewBeanCollection);
