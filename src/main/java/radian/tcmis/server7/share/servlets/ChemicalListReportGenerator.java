...
import radian.tcmis.server7.share.helpers.ChemicalList;
import radian.tcmis.both1.reports.RegisterTable;

public class ChemicalListReportGenerator {
    //ACJEngine
    ...
    public ChemicalListReportGenerator() {
    }

  public String buildReport(Vector chemicalData, String listID, String listDesc) {
    String url = "";
    ...
    try {
      ...
      erw.readTemplate(""+templatpath+"ChemicalList.erw");

      //modifying template
      tm = erw.getTemplateManager();
      tm.setLabel("LBL010",listID);
      tm.setLabel("LBL014",listDesc);
	  ...
	  erw.setCacheOption(true, ""+tempwritefilepath+"ChemicalList"+tmpReqNum.toString()+".joi");
      erw.setDataSource(ds);
      //generating report
      ec.setReportData(erw.generateReport());
      ec.setPDFProperty("FileName",""+tempwritefilepath+"ChemicalList"+tmpReqNum.toString()+".pdf");
      ...
      url = tempurlpath + "ChemicalList"+tmpReqNum.toString()+".pdf";
    }catch (Exception e) {
      e.printStackTrace();
      url = "";
    }
  ...
  public RegisterTable[] getData(Vector reportData1) throws Exception{
      ...
      try {
        rt[0] = new RegisterTable(ChemicalList.getVector(reportData1),"CHEMICALLIST",ChemicalList.getFieldVector(),null);
      }catch(Exception e1) {
        e1.printStackTrace();
        throw e1;
      }
