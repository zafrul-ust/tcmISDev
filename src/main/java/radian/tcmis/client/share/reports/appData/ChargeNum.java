...
public class ChargeNum {
  ...
  public ChargeNum(String item, String cnum1,String cnum2,String percent){
  ...
  public static Vector getVector(Vector lineItems){
    ...
    for(int i=0;i<lineItems.size();i++){
      ...
      for(int j=0;j<v2.size();j++){
        ...
        hasOne = true;
        v.addElement(new ChargeNum(item,cnum1,cnum2,percentage));
      }
      if(!hasOne){
System.out.println("doesn't have one");
        v.addElement(new ChargeNum(item));
      }
    }
    // big,stinkin',kludge
