...
public class ChemSelectDlg extends JDialog {
  CmisApp cmis;
  String listId;
  String listDesc;
  ...
  public ChemSelectDlg(CmisApp cmis) {
    super(cmis.getMain(), "Select a Chemical", true);
    this.cmis = cmis;
    try  {
      jbInit();
      pack();
  ...
  void jbInit() throws Exception {
    ...
    */
    ...
    jLabel1.setText("Find Chemicals matching:");
  ...
  void getData(){
    ChemSelectDlgLoadThread c = new ChemSelectDlgLoadThread(this);
    c.start();
  }
  void loadData(){
    ...
    try{
      ...
      query.put("ACTION","SELECT_CHEMICAL");
  ...
  void buildTable(Vector v,Hashtable Result1){
   System.out.println("This is the table in chem select dlg "+Result1);
    jPanel1.removeAll();
    //jsp.removeAll();
    jsp = new JScrollPane();
    ...
    if(v.size()==0){
      ...
      gd.setMsg("There were no chemicals that matched the search criteria.");
  ...
  void okB_actionPerformed(ActionEvent e) {
    ...
    if(myTable.getSelectedRowCount()<1){
      GenericDlg gd = new GenericDlg(cmis.getMain(),"Select a Chemical",true);
      gd.setMsg("Please select a chemical.");
      CenterComponent.centerCompOnScreen(gd);
      gd.setVisible(true);
      return;
    }
    casNum = myTable.getValueAt(myTable.getSelectedRow(),0).toString();
...
class ChemSelectDlgLoadThread extends Thread {
  ChemSelectDlg parent;
  public ChemSelectDlgLoadThread(ChemSelectDlg parent){
     super("ChemSelectDlgLoadThread");
     this.parent = parent;
  }
  public void run(){
     parent.loadData();
