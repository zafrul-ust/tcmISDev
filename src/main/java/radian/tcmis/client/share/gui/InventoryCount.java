...
public class InventoryCount extends JPanel {
  ...
  public InventoryCount( CmisApp cmis )
  {
    try
    {
      this.cmis=cmis;
  ...
  public void loadIt()
  {
    InventoryCountLoadThread x=new InventoryCountLoadThread( this );
    x.start();
  }
  ...
  void jbInit() throws Exception {
    ...
    statusL7.setText( "7. Click the OK button.'" );
  ...
  void runThread()
  {
    InventoryCountThread st=new InventoryCountThread( this );
    st.start();
  }
  ...
  public void douploadstuff()
  {
    ...
    uploadfile.append( "NO|001|SR|1" + linefeedd );
    uploadfile.append( "IF|&99|=|0|+1|ERROR6" + linefeedd );
    uploadfile.append( "NO|001|TT" + linefeedd );
    uploadfile.append( "NO|001|FR|A.DAT|hubcount.csv" + linefeedd );
    uploadfile.append( "IF|&99|=|0|+1|ERROR7" + linefeedd );
    uploadfile.append( "NO|001|FR|B|nolabel.csv" + linefeedd );
    uploadfile.append( "IF|&99|=|0|+1|ERROR8" + linefeedd );
    uploadfile.append( "SK|RESETFILES" + linefeedd );
    uploadfile.append( "LB|ERROR7" + linefeedd );
    uploadfile.append( "FC|ERROR7.TXT|LINKERR.TXT" + linefeedd );
    ...
    uploadfile.append( "LB|RESETFILES" + linefeedd );
    uploadfile.append( "NO|001|FD|A" + linefeedd );
    uploadfile.append( "NO|001|FD|B" + linefeedd );
    ...
    if ( scrpitresult )
    {
      ...
      {
        // first change
        BufferedReader in=new BufferedReader( new FileReader( "c:\\scannerdownload\\hubcount.csv" ) );
        ...
        TcmisHttpConnection cgi=new TcmisHttpConnection( "hubUpload",cmis );
  ...
  try
    {
      ...
      // any error message?
      StreamGobblerInventoryCount errorGobbler=new StreamGobblerInventoryCount( proc.getErrorStream(),"ERROR" );

      // any output?
      StreamGobblerInventoryCount outputGobbler=new StreamGobblerInventoryCount( proc.getInputStream(),"OUTPUT" );
...
class InventoryCountThread extends Thread {
  InventoryCount parent;
  boolean runF = true;

  public InventoryCountThread (InventoryCount parent){
     super("InventoryCountThread");
     this.parent = parent;
  }
...
class StreamGobblerInventoryCount extends Thread
{
    InputStream is;
    String type;

    StreamGobblerInventoryCount(InputStream is, String type)
    {
        this.is = is;
        this.type = type;
    }
...
class InventoryCountLoadThread extends Thread {
  InventoryCount parent;
  public InventoryCountLoadThread(InventoryCount parent){
     super("InventoryCountLoadThread");
     this.parent = parent;
  }
  public void run(){
     parent.loadItAction();
