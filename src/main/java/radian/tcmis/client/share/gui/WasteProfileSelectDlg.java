...
public class WasteProfileSelectDlg extends JDialog {
  CmisApp cmis;
  String listId;
  String listDesc;
  ...
  String profileDesc = "";
  String profileID = "";
  String[] colHead = new String[]{"Profile","Description"};
  StaticJLabel jLabel2 = new StaticJLabel();
  JComboBox vendorC = new JComboBox();
  Vector vID = null;
  String vendorID = null;

  public WasteProfileSelectDlg(CmisApp cmis, String vendor) {
    super(cmis.getMain(), "Select a Profile for: "+vendor, true);
    this.cmis = cmis;
    try  {
      jbInit();
      pack();
  ...
  void loadItAction(){
    ...
    try{
      ...
      Hashtable h = (Hashtable)result.get("VENDOR_INFO");
      vID = (Vector)h.get("VENDOR_ID");
      vDesc = (Vector)h.get("VENDOR_NAME");
      ...
      if (vID.size() < 1) {
        ...
        vendorC = ClientHelpObjs.loadChoiceFromVector(vendorC,vDesc);
      }
      this.repaint();
  ...
  void loadData(){
    ...
    try{
      TcmisHttpConnection cgi = new TcmisHttpConnection("WasteAhocReport",cmis);
      Hashtable query = new Hashtable();
      query.put("ACTION","SEARCH_WASTE_PROFILE");
      ...
      v = (Vector)result.get("WASTE_PROFILE_INFO");
      if (v.size() < 1) {
        GenericDlg gd = new GenericDlg(cmis.getMain(),"No Record",true);
        gd.setMsg("No record found.");
        CenterComponent.centerCompOnScreen(gd);
  ...
  void buildTable(Vector v,Hashtable Result1){
    ...
    for(int i=0;i<v.size();i++){
      ...
      oa[i][0] = h.get("VENDOR_PROFILE_ID").toString();
    ...
    if(v.size()==0){
      ...
      gd.setMsg("There were no profile that matched the search criteria.");
      CenterComponent.centerCompOnScreen(gd);
      gd.setVisible(true);
    }
  }
  ...
  void okB_actionPerformed(ActionEvent e) {
    ...
    if(myTable.getSelectedRowCount()<1){
      GenericDlg gd = new GenericDlg(cmis.getMain(),"Select a profile",true);
      gd.setMsg("Please select a profile.");
      CenterComponent.centerCompOnScreen(gd);
      gd.setVisible(true);
      return;
    }
    profileID = myTable.getValueAt(myTable.getSelectedRow(),0).toString();
    profileDesc = myTable.getValueAt(myTable.getSelectedRow(),1).toString();
...
void searchB_actionPerformed(ActionEvent e) {
    ClientHelpObjs.playSound(ClientHelpObjs.SOUND_BUTTON_ACTION);
    if (vendorC.getSelectedItem().toString().startsWith("Select")) {
