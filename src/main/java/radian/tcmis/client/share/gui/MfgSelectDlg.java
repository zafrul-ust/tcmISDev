...
public class MfgSelectDlg extends JDialog {
  CmisApp cmis;
  String listId;
  String listDesc;
  ...
  String mfgDesc = "";
  String mfgID = "";
  String[] colHead = new String[]{"ID","Manufacturer Name"};

  public MfgSelectDlg(CmisApp cmis) {
    super(cmis.getMain(), "Select a Manufacturer", true);
    this.cmis = cmis;
    try  {
      jbInit();
      pack();
  ...
  void jbInit() throws Exception {
    ...
    */
    ...
    jLabel1.setText("Find Manufacturer matching:");
  ...
  void getData(){
    /*
    MfgSelectDlgLoadThread c = new MfgSelectDlgLoadThread(this);
    c.start(); */
    loadData();
  }
  void loadData(){
    ...
    try{
      ...
      query.put("ACTION","SEARCH_MANUFACTURER");
      ...
      v = (Vector)result.get("MANUFACTURER_INFO");
      if (v.size() < 1) {
  ...
  void buildTable(Vector v,Hashtable Result1){
    ...
    for(int i=0;i<v.size();i++){
      ...
      oa[i][0] = h.get("MANUFACTURER_ID").toString();
      oa[i][1] = h.get("MANUFACTURER_DESC").toString();
    }
    ...
    if(v.size()==0){
      ...
      gd.setMsg("There were no manufacturer that matched the search criteria.");
      CenterComponent.centerCompOnScreen(gd);
      gd.setVisible(true);
    }
  }
  ...
  void okB_actionPerformed(ActionEvent e) {
    ...
    if(myTable.getSelectedRowCount()<1){
      GenericDlg gd = new GenericDlg(cmis.getMain(),"Select a manufacturer",true);
      gd.setMsg("Please select a manufacturer.");
      CenterComponent.centerCompOnScreen(gd);
      gd.setVisible(true);
      return;
    }
    mfgID = myTable.getValueAt(myTable.getSelectedRow(),0).toString();
    mfgDesc = myTable.getValueAt(myTable.getSelectedRow(),1).toString();
...
class MfgSelectDlgLoadThread extends Thread {
  MfgSelectDlg parent;
  public MfgSelectDlgLoadThread(MfgSelectDlg parent){
     super("MfgSelectDlgLoadThread");
     this.parent = parent;
  }
  public void run(){
     parent.loadData();
