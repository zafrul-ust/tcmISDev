...
public class PartNumberCommentsDlg extends JDialog {
  CmisApp cmis;
  String partNo = "";
  String partGroupNo = "";
  String catalogID = "";
  String catalogCompanyId = "";
  boolean editableByApprover = false;
  String userName = "";
  ...
  public PartNumberCommentsDlg(CmisApp cmis, String catalogID, String partNo, String partGroupNo, boolean editableByApprover, String catalogCompanyId) {
    super(cmis.getMain(), "Part Number Comments", true);
    this.cmis = cmis;
    this.partNo = partNo;
  ...
  void jbInit() throws Exception {
    ...
    processB.setFont(partL.getFont());
    ...
    textP.add(partL,  new GridBagConstraints(0, 1, 1, 1, 0.0, 0.0
          ,GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets(5, 5, 5, 5), 0, 0));
  ...
  void getData(){
    PartNumberCommentsDlgLoadThread c = new PartNumberCommentsDlgLoadThread(this);
    c.start();
  }
  ...
  void loadData(){
    ...
    try{
      ...
      query.put("ACTION","GET_PART_NUMBER_COMMENTS");
      ...
      if (success.booleanValue()) {
        Hashtable h = (Hashtable)result.get("PART_NUMBER_COMMENTS");
        //first set header info
        catalogT.setText((String)h.get("CATALOG_DESC"));
        partNoT.setText(partNo);
        userName = (String)h.get("USER_NAME");
        //now build table
        Vector dataV = (Vector) h.get("PART_NUMBER_COMMENT_DATA");
  ...
  void buildTable(int tableSize, Vector dataV) {
    ...
    displayTable.addMouseListener(new PartNumberComments_mouseAdapter(this));
  ...
  void processB_actionPerformed(ActionEvent e) {
    ...
    try{
      ...
      TcmisHttpConnection cgi = new TcmisHttpConnection("ReportObj",cmis);
      Hashtable query = new Hashtable();
      query.put("ACTION","PROCESS_PART_NUMBER_COMMENTS");
      ...
      query.put("PART_NUMBER_COMMENTS_DATA",dataV);
      Hashtable result = cgi.getResultHash(query);
      if(result == null){
        GenericDlg.showAccessDenied(cmis.getMain());
        return;
  ...
  void editB_actionPerformed(ActionEvent e) {
    ...
    if (!cmis.getUserId().equals(tempUserID) && !editableByApprover) {
  ...
  void printB_actionPerformed(ActionEvent e) {
    ClientHelpObjs.playSound(ClientHelpObjs.SOUND_BUTTON_ACTION);
      try{
        ...
        TcmisHttpConnection cgi = new TcmisHttpConnection("ReportObj",cmis);
        Hashtable query = new Hashtable();
        query.put("ACTION","PRINT_PART_NUMBER_COMMENTS");
        query.put("USER_ID",cmis.getUserId());
...
class PartNumberCommentsDlgLoadThread extends Thread {
  PartNumberCommentsDlg parent;
  public PartNumberCommentsDlgLoadThread(PartNumberCommentsDlg parent){
     super("PartNumberCommentsDlgLoadThread");
     this.parent = parent;
  }
  public void run(){
     parent.loadData();
...
class PartNumberComments_mouseAdapter extends java.awt.event.MouseAdapter {
  PartNumberCommentsDlg adaptee;
  PartNumberComments_mouseAdapter(PartNumberCommentsDlg adaptee) {
