   ...
   db = new RayTcmISDBModel("Tcm_Ops","2",radian.web.HTMLHelpObj.getTcmISLocaleString(request));
	 ...
	 if (! (auth.equalsIgnoreCase("authenticated"))) {
       ...
       if( localeCode !=null && localeCode.length() == 5)
       {
		...
		String totalCostCurrencyId = request.getParameter("totalCostCurrencyId");
		...
		try {
		 out.println(obj1.buildPage(ponumber, blanketpo, logonName, addChargesList, db,
			headerNote, personnelId,localeObject,localeCode,totalCost,totalCostCurrencyId));
		}
		catch (Exception ee) {
		 ee.printStackTrace();
		}
