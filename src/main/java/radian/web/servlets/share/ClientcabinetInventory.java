...
public class ClientcabinetInventory
{
  ...
  private static org.apache.log4j.Logger cabInvLog = org.apache.log4j.Logger.getLogger(purchaseOrderCreator.class);
  ...
  public ClientcabinetInventory ( ServerResourceBundle b,TcmISDBModel d )
  {
    bundle=b;
    db=d;
  }
  ...
  if ( cabsStringfromArray.length() == 0 )
  {
    ...
    try
    {
      ...
      if ( initialDataLoaded.equalsIgnoreCase( "Yes" ) )
      {
        ...
        hub_list_set  = (Hashtable)relabelsession.getAttribute("CUSTOMER_HUB_SET");
      }
      else
      {
	Hashtable hub_list_set1=null;
	Hashtable hub_list1=null;

	hub_list_set1=radian.web.HTMLHelpObj.createCustomerHubList( db );
	relabelsession.setAttribute( "CUSTOMER_HUB_SET",hub_list_set1 );
	hub_list1 = (Hashtable)(hub_list_set1.get("HUB_LIST"));
	...
	relabelsession.setAttribute( "INITIAL_CABINET_LABEL_DATA",initialData );
        ...
        relabelsession.setAttribute( "CLIENT_CABINET_DATA_LOADED","Yes" );
	...
	else
	{
	  ...
	  String faccompanyId = (String) faccompdetails.get(facility);
        ...
        buildHeader( searchwhat,containsorlike,searchtext,facility,workarea,initialData," ",sortby,hub_list_set,selectedcabs,showncabs,selHub,expfrom,expto,relabelsession);
   ...
   private String createComboBoxData( Hashtable initialData )
     ...
     private StringBuffer buildxlsDetails( Vector data,String SubuserAction,String personnelID ) throws Exception
     {
       ...
       try
       {
         ...
         Msg.append( "Item,Description,DPM,Part Num,Work Area,Cabinet,Receipt ID,Mfg Lot, Expire Date,Last Count Date,Last Count Qty,Delivered Since Last Count,Total Qty,Unit Cost,Reorder Point,Stocking Level,Lead Time in Days\n" );
     ...
     private void buildDetails( Vector data,String hubnum)
     {
       //StringBuffer Msg=new StringBuffer();
	   ...
	   out.println( "<TH width=\"5%\"  height=\"38\">Open MR Qty</TH>\n" );
	...
	//	HUB, BIN_ID, CABINET_ID, CABINET_NAME, FACILITY_ID, ORDERING_APPLICATION, USE_APPLICATION, COMPANY_ID, RECEIPT_ID, MFG_LOT, EXPIRE_DATE, COUNT_DATETIME, COUNTED_BY_PERSONNEL_ID, COUNTED_BY_NAME, COUNT_QUANTITY, ITEM_ID, ITEM_DESC, QTY_ISSUED_AFTER_COUNT, TOTAL_QUANTITY
	   ...
	   String stocking_level = hD.get("STOCKING_LEVEL")==null?"&nbsp;":hD.get("STOCKING_LEVEL").toString();
	   String qty_available_after_alloc = hD.get("qty_available_after_alloc")==null?"&nbsp;":hD.get("qty_available_after_alloc").toString();
	   String open_mr_qty = hD.get("open_mr_qty")==null?"&nbsp;":hD.get("open_mr_qty").toString();
		 String leadtimedays = hD.get("LEAD_TIME_DAYS")==null?"":hD.get("LEAD_TIME_DAYS").toString();
	   ...
	   out.println("<tr align=\"center\">\n");
	   out.println("<td "+Color+"\" width=\"5%\" height=\"38\"><A HREF=\"javascript:showinventorydetail('"+item_id+"','"+hub+"')\" STYLE=\"color:#0000ff\">"+item_id+"</A></td>\n");
	   out.println("<td "+Color+"\" width=\"15%\" height=\"38\">"+item_desc+"</td>\n");
	   if ("2118".equalsIgnoreCase(hubnum))
	   {
...
/*	   if (!"0".equalsIgnoreCase(qty_available_after_alloc))
	   {
		 out.println("<td "+Color+"\" width=\"5%\" height=\"38\"><A HREF=\"javascript:showinventorydetail('"+item_id+"','"+hub+"')\" STYLE=\"color:#0000ff\">"+qty_available_after_alloc+"</A></td>\n");
	   }
	   else
	   {
	   ...
	   out.println("<td "+Color+"\" width=\"5%\" height=\"38\">"+open_mr_qty+"</td>\n");
    ...
    private void buildHeader(String searchLike,String containsorlke,String searchtext,String selectedFac,String tmpwaSelect, Hashtable initialData,String buildlabels,
                                     String sortby,Hashtable hubsetdata,Vector selectedcabs,Vector showncabs,String selectedHub,String expiresfrom,
									 String expiresto,HttpSession relabelsession)
    {
	  HeaderFooter hf=new HeaderFooter( bundle,db );
	  out.print(hf.printHTMLHeader("tcmIS Cabinet Inventory",intcmIsApplication));
	  out.print( "<SCRIPT SRC=\"/clientscripts/cabinetinv.js\" LANGUAGE=\"JavaScript\"></SCRIPT>\n" );
   	  out.print( hf.printTopToolBar( "","cabinetinventory.gif",relabelsession ) );

	  String Search_servlet=  bundle.getString( "CABINET_INVENTORY_SERVLET" ) == null ? "" :  bundle.getString( "CABINET_INVENTORY_SERVLET" ).toString();
...
//    out.println( "&nbsp;\n" );
      ...
      out.println("<TD WIDTH=\"5%\" ALIGN=\"LEFT\" VALIGN=\"MIDDLE\">\n");
      out.println("</TD>\n");
       ...
       throws Exception
    {
	  ...
	  try
	  {
		...
		try
		{
		  cs=connect1.prepareCall( "{call PKG_CABINET_INV_COUNT_REPORT.PR_CAB_INV_COUNT_REP_CLIENT (?,?,?,?,?,?,?,?,?,?,?,?)}" );
		  cs.setString( 1,sourcehub );
		  if ( facSelected.length() > 0 && ( !"All Facilities".equalsIgnoreCase( facSelected ) ) )
		  {
			cs.setString( 2,facSelected );
		  ...
		  if ( searchtext.length() > 0 )
		  {
			...
			cs.setString( 6,searchLike );
			cs.setString( 7,searchtext );
		  }
		  else
		  {
			cs.setNull( 5,java.sql.Types.VARCHAR );
		  ...
		  if (expirefrom.trim().length() > 0){cs.setInt(8,Integer.parseInt(expirefrom));}else{cs.setNull(8,java.sql.Types.VARCHAR);}
		  if (expireto.trim().length() > 0){cs.setInt(9,Integer.parseInt(expireto));}else{cs.setNull(9,java.sql.Types.VARCHAR);}
    ...
    cabInvLog.debug(query);
    searchRs= ( ResultSet ) cs.getObject( 11 );
		  ...
		  while ( searchRs.next() )
		  {
			...
			//open_mr_qty
			DataH.put("open_mr_qty",searchRs.getString("open_mr_qty")==null?"":searchRs.getString("open_mr_qty"));
			//LEAD_TIME_DAYS
		...
		{
		  e.printStackTrace();
		  radian.web.emailHelpObj.senddatabaseHelpemails( "Error Calling Procedure PKG_CABINET_INV_COUNT_REPORT.PR_CAB_INV_COUNT_REP_CLIENT in Clinet Cabinet Inventory",
														  "Error occured while running call PKG_CABINET_INV_COUNT_REPORT.PR_CAB_INV_COUNT_REP_CLIENT \n Facility "+facSelected+"\n companyID "+companyID+"\n Error Msg:\n" + e.getMessage() +"" );
		  throw e;
		}
		finally
		{
