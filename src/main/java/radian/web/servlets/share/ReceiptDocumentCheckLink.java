...
public class ReceiptDocumentCheckLink extends HttpServlet implements SingleThreadModel
{
      private ServerResourceBundle bundle=null;
      private TcmISDBModel db = null;

      public ReceiptDocumentCheckLink(ServerResourceBundle b, TcmISDBModel d)
      {
          bundle = b;
          db = d;
      }

      public ReceiptDocumentCheckLink()
      {
    ...
    public void doPost(HttpServletRequest request,HttpServletResponse response,String client) throws ServletException,IOException
    {
        ...
        query = query + "order by material_id,revision_date desc ";
 			  ...
 			  query += "FROM GLOBAL.msds m WHERE m.on_line = 'Y' and material_id > 40000 ORDER BY material_id, revision_date DESC  ";

//Check for Recipet Documents
				 query = "SELECT DISTINCT m.ENTERED_BY_NAME REVIEWED_BY1 ,to_char(m.INSERT_DATE,'mm/dd/yyyy') REVIEW_DATE1, 'Y' on_line, DOCUMENT_URL content, '' content11, ";
				 query += "m.DOCUMENT_DATE REVISION_DATE,TO_CHAR (m.DOCUMENT_DATE, 'mm/dd/yyyy') revision_date1, RECEIPT_ID material_id, ' ' cat_part_no, ' ' catalog_id, '' mfg_desc,";
				 query += "'Y' msds_on_line, ' ' trade_name, ' ' facility_id,INSERT_DATE	 ";
				 query += "FROM RECEIPT_DOCUMENT_VIEW m WHERE insert_date > sysdate-150 ORDER BY   RECEIPT_ID,INSERT_DATE";
        ...
        //System.out.println("Done: Total Records: "+testTotal+" Vector Size  "+ResultF.size());
    ...
    public StringBuffer buildHTML(Vector ResultFinal,String clientname) throws IOException
    {
        ...
        Msg1.append("<TR>");
        Msg1.append("<TD><B>RECEIPT ID</B></TD><TD><B>Document DATE</B></TD><TD><B>Entered BY</B></TD><TD><B>Enter DATE</B></TD><TD><B>ACTIVE</B></TD><TD><B>CONTENT</B></TD><TD><B>LATEST?</B></TD><TD><B>ONLY REVISION DATE</B></TD>");
        Msg1.append("</TR>\n");
        ...
        for ( int j=0; j < ResultFinal.size(); j++ )
        {
          ...
          for ( int i=0; i < ResultV1.size(); i++ )
          {
            ...
            try
            {
				...
				if ( isss == 404 )
				{
				  //System.out.println( "Not Found Content, " + Finaop.get( "MATERIAL_ID" ) + ", " + Finaop.get( "REVISION_DATE" ) + " , " + Content);
            ...
            {
              //System.out.println( "Not Found Content, " + Finaop.get( "MATERIAL_ID" ) + ", "+Finaop.get( "REVISION_DATE" )+" , " + Content + " , "+e.getMessage() );
              Msg1.append( "<TR><TD ALIGN=\"LEFT\">" );
              Msg1.append( Finaop.get( "MATERIAL_ID" ) );
              Msg1.append( "</TD><TD ALIGN=\"LEFT\">" );
              Msg1.append( Finaop.get( "REVISION_DATE1" ) );
		...
		String temprpturl =radian.web.tcmisResourceLoader.gettempreporturl();
