...
public class showFlowdowns
{
  ...
  public showFlowdowns( ServerResourceBundle b,TcmISDBModel d )
  {
    bundle=b;
    db=d;
  }
     ...
     throws ServletException,IOException
  {
    ...
    //System.out.println( "Here itemID  " + itemID + " Action   " + Action + "  searchString  " + searchString + " lineID " + lineID + "   ammendMent " + ammendMent );
    if ( "lookaheadview".equalsIgnoreCase( Action ) )
    {
      buildsendLookahead( lineID,itemID,radianPO,out );
    }
    else if ( "view".equalsIgnoreCase( Action ) )
    {
      buildsendflowdowns( lineID,itemID,radianPO,radianBPO,shiptTo,ammendMent,shiptocompanyid,out );
    }

    out.close();
  }

  public void buildsendLookahead( String lineID,String itemID,String radianpo,PrintWriter poout )
  {
    //StringBuffer Msgn=new StringBuffer();
    //StringBuffer Msgbody=new StringBuffer();

    nematid_Servlet=bundle.getString( "PURCHASE_ORDER_SHOWFLOWDOWNS" );

    poout.println( radian.web.HTMLHelpObj.printHTMLHeader( "Purchase Order Flow Downs","",false ) );
    ...
    radian.web.poHelpObj.buildsendLookahead( db,"yes",lineID,itemID,radianpo,canupdate,poout,res);
  ...
  public void buildsendflowdowns( String lineID,String itemID,String radianpo,String radianbpo,String shipToLoc,String ammenNum,String shiptocompanyid,PrintWriter poout )
  {
    //StringBuffer Msgn=new StringBuffer();
    //StringBuffer Msgbody=new StringBuffer();
    ...
    poout.println( "function sendSupplierData()\n" );
    poout.println( "{\n" );
    radian.web.poHelpObj.buildsendFlowdowns( db,"",lineID,itemID,radianpo,radianbpo,shipToLoc,ammenNum,shiptocompanyid,poout,res );
    poout.println( "window.close();\n" );
    poout.println( " }\n" );
    poout.println( "// -->\n</SCRIPT>\n" );
    //poout.println( Msgtemp );
