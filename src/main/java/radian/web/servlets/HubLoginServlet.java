 ...
 * 12-15-03 - Not used
 */
public class HubLoginServlet extends HttpServlet
{
 ...
 * value 'challenge' and the login form is redisplayed.
        ...
        throws ServletException, IOException
    {
        ...
        //try{ companyID = request.getParameter("companyID").toString();}catch (Exception e){ companyID = "Radian";}
        String CompanyID = session.getValue("COMPANYID").toString();
        ...
        if( "challenge".equals(state) && loginId != null && passwd != null )
        {
            ...
            try
            {
                Password pw = new Password(db1, loginId, passwd,CompanyID);

                session.setAttribute("PERSONNELID", pw.getUserId());

                //System.out.println("Personnel Id " +pw.getUserId());

                int test_number = DbHelpers.countQuery(db1,"select count(*) from user_group_member where user_group_id = 'Receiving' and personnel_id = '"+pw.getUserId()+"' and company_id = '"+CompanyID+"'");

                int test_qc_number = DbHelpers.countQuery(db1,"select count(*) from user_group_member where user_group_id = 'Receiving QC' and personnel_id = '"+pw.getUserId()+"' and company_id = '"+CompanyID+"'");

                int test_numberInven = DbHelpers.countQuery(db1,"select count(*) from user_group_member where user_group_id = 'Inventory' and personnel_id = '"+pw.getUserId()+"' and company_id = '"+CompanyID+"'");
