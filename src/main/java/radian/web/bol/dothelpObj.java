 ...
 * 02-05-07 Escape ' in TECHNICAL_NAME
 */
...
public class dothelpObj {
    ...
    public dothelpObj(TcmISDBModel db ){
        this.db = db;
        vMsdsNoList = new Vector();
    }
	...
	public Hashtable getMSDSData( String item_id )
	{
	  ...
	  String msdsdetailquery=" select SCHEDULE_B_DESC,SCHEDULE_B,PROPER_SHIPPING_NAME,HAZMAT_ID, HAZMAT_TECHNICAL_NAME,PART_ID, MATERIAL_ID, ITEM_ID, PKG_GT_RQ, TEST_PKG_GT_RQ, BULK_PKG_MARINE_POLLUTANT, ORM_D, WEIGHT_LB, MATERIAL_DESC, ";
	  msdsdetailquery += "HAZARD_CLASS, SUBSIDIARY_HAZARD_CLASS, PACKING_GROUP, IDENTIFICATION_NUMBER UN_NUMBER, DRY_ICE, ERG,MARINE_POLLUTANT, REPORTABLE_QUANTITY_LB, ";
	  msdsdetailquery += "SHIPPING_INFO_REMARKS, PACKAGING, LAST_CHANGED_BY, LAST_UPDATED from dot_part_material_view where item_id= " + item_id + " order by PART_ID asc ";
	  ...
	  try
	  {
       ...
       while ( rs.next() )
       {
         ...
         MaterialData.put( "GROUND_SHIPPING_NAME",rs.getString( "PROPER_SHIPPING_NAME" ) == null ? "" : rs.getString( "PROPER_SHIPPING_NAME" ) );
		 ...
		 MaterialData.put( "HAZMAT_ID",rs.getString( "HAZMAT_ID" ) == null ? "" : rs.getString( "HAZMAT_ID" ) );
		 MaterialData.put( "TECHNICAL_NAME",rs.getString( "HAZMAT_TECHNICAL_NAME" ) == null ? "" : rs.getString( "HAZMAT_TECHNICAL_NAME" ) );
		 MaterialData.put( "SYMBOL","" );
		 MaterialData.put( "SCHEDULE_B",rs.getString( "SCHEDULE_B" ) == null ? "" : rs.getString( "SCHEDULE_B" ) );
		 MaterialData.put( "SCHEDULE_B_DESC",rs.getString( "SCHEDULE_B_DESC" ) == null ? "" : rs.getString( "SCHEDULE_B_DESC" ) );
  ...
  public boolean updateMsdsData( Hashtable DataH,String loginID,String action )
  {
	...
	String symbol=materialData.get( "SYMBOL" ).toString();
	String hazmat_id=materialData.get( "HAZMAT_ID" ).toString();
	String technical_name=materialData.get( "TECHNICAL_NAME" ).toString();
	technical_name = BothHelpObjs.changeSingleQuotetoTwoSingleQuote(technical_name);
    ...
    try
    {
      ...
      if ( material_id.length() > 0 )
      {
		...
		if ("G".equalsIgnoreCase(symbol) || "A G".equalsIgnoreCase(symbol) || "D G".equalsIgnoreCase(symbol) || "G I".equalsIgnoreCase(symbol))
		{
		  materialquery+=" HAZMAT_TECHNICAL_NAME='" + technical_name + "',";
		}
		else
		{
		  materialquery+=" HAZMAT_TECHNICAL_NAME=null,";
		}


		if (hazmat_id.length() > 0)
		{
		  materialquery+=" HAZMAT_ID=" + hazmat_id + ",";
		}
		else
		{
		  materialquery+=" HAZMAT_ID=null,";
		}
        ...
        {
          materialquery+=" PROPER_SHIPPING_NAME='" + ground_shipping_name + "',";
        }
        else
        {
          materialquery+=" PROPER_SHIPPING_NAME=null,";
        }
        ...
        {
          materialquery+=" IDENTIFICATION_NUMBER='" + un_number + "',";
        }
        else
        {
        materialquery+=" IDENTIFICATION_NUMBER=null,";
        }*/
        ...
        materialquery+=" SHIP_CHANGE_ID="+loginID+",MARINE_POLLUTANT='"+marine_pollutant+"',SHIPPING_INFO_REMARK='"+shipping_info_remarks+"'";
